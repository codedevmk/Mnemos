...
### Repository setup
- [ ] Initialize monorepo `mnemos/` with the directory layout from TDS §5.
- [ ] Add `LICENSE` (Apache-2.0) at root.
- [ ] Add `LICENSE-chips` (MIT) at root.
- [ ] Add `THIRD_PARTY_NOTICES.md` (empty template).
- [ ] Add `README.md` with one-paragraph vision blurb and pointer to docs.
- [ ] Add `CONTRIBUTING.md` (placeholder; populated by M9).
- [ ] Add `CODE_OF_CONDUCT.md`.
- [ ] Add `SECURITY.md` (placeholder).

### Source control hygiene
- [ ] Add `.gitignore` that rejects `out/`, `build/`, `*.log`, `*.bin` (firmware), IDE caches, vcpkg/Conan artifacts.
- [ ] Add `.gitattributes` (line endings, binary classifications).
- [ ] Add `.editorconfig`.

### Formatting and linting
- [ ] Add `.clang-format` (LLVM base + project overrides; column limit, brace style decided here).
- [ ] Add `.clang-tidy` with curated check list (modernize-*, bugprone-*, performance-*, readability-* subset).
- [ ] Add pre-commit hook script (`tools/git-hooks/pre-commit`) running clang-format dry-run.
- [ ] Add `tools/install-hooks.sh` and `tools/install-hooks.ps1`.

### Build system
- [ ] Author root `CMakeLists.txt` with project declaration, C++23 baseline, global warning flags.
- [ ] Author `CMakePresets.json` covering all eight presets from TDS §6.3.
- [ ] Author `cmake/modules/MnemosDeclareTier.cmake` exposing `mnemos_declare_tier()`.
- [ ] Author `cmake/modules/MnemosCompilerFlags.cmake` with the per-platform flag bundle from TDS §6.4.
- [ ] Author `cmake/modules/MnemosTesting.cmake` providing `mnemos_add_test()`.
- [ ] Enforce out-of-source builds at root CMakeLists.
- [ ] Add empty `extern/` with `README.md` documenting FetchContent usage convention.

### Foundation skeleton
- [ ] Create `foundation/CMakeLists.txt` declaring `mnemos::foundation` as a header-only target with one no-op header.
- [ ] Add one trivial unit test under `foundation/tests/` proving Catch2 wiring works.
- [ ] FetchContent integration for Catch2 v3 in `cmake/modules/`.

### Continuous Integration
- [ ] Choose CI provider (GitHub Actions is the default; deviation requires an ADR).
- [ ] Add `.github/workflows/ci.yml` with matrix: {windows-latest, ubuntu-24.04} × {Debug, Release}.
- [ ] CI runs configure + build + test for each preset.
- [ ] CI fails on any compiler warning.
...
- [ ] Create `docs/adr/0001-monorepo-layout.md` recording the layout decision.
- [ ] Create `docs/adr/0002-cmake-and-toolchain.md` recording build system choices.
- [ ] Create `docs/adr/0003-license-split.md` recording Apache+MIT rationale.
...
- [ ] Create `chips/common/` library target `mnemos::chips::common`.
...
- [ ] Create `chips/cpu/m6510/` library target `mnemos::chips::cpu::m6510`.
...
### VIC-II 6569
- [ ] Create `chips/video/vic_ii_6569/` library target.
...
### SID 6581
- [ ] Create `chips/audio/sid_6581/` library target.
...
- [ ] Reference: at least one published SID emulation note set documented in `chips/audio/sid_6581/NOTES.md`.

### CIA 6526
- [ ] Create `chips/bus_controller/cia_6526/` library target.
...
- [ ] Create `chips/mapper/c64_pla/` library target.
...
### Topology library
- [ ] Create `topology/` library target `mnemos::topology`.
...
### Manifest loader
- [ ] Create `manifests/common/` library target `mnemos::manifests::common`.
...
### C64 manifest
- [ ] Create `manifests/c64/` directory.
- [ ] Author `c64.pal.toml` per TDS §10.2.
- [ ] Document ROM acquisition and SHA-256s in `manifests/c64/ROMS.md`.
- [ ] Note: ROM files themselves are NOT committed; CI obtains them from a configured source.

### Runtime library
- [ ] Create `runtime/` library target `mnemos::runtime`.
...
- [ ] Author Cap'n Proto schemas under `instrumentation/wire/`.

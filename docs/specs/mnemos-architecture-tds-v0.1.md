...
├── extern/                        # Vendored or FetchContent'd third-party
├── foundation/                    # Tier 1
...
- All build artifacts MUST go under `out/`. The root directory MUST stay clean.
- All test logs MUST go under `out/logs/`.
- `.gitignore` MUST reject `*.log`, `*.bin` (firmware dumps), `out/`, IDE caches, and CMake build trees outside `out/`.
...
- Root `CMakeLists.txt` declares the project, language baseline, global flags, and adds each tier as a subdirectory in dependency order.
...
- MIT for the chip library (`chips/`).
...
- Schemas live in `instrumentation/wire/*.capnp` and generate C++ + Python bindings at build time.

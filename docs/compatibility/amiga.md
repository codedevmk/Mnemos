...
| Rainbow Islands | TBD | Operational (visual smoke) | TBD | TBD | TBD | Operational (visual smoke) | Operational (visual smoke) | TBD | TBD | TBD |
...
| Turrican (Europe) | TBD | Operational (visual smoke) | TBD | TBD | TBD | Operational (visual smoke) | Operational (visual smoke) | TBD | TBD | TBD |
...
| Date | Evidence |
| --- | --- |
| 2026-07-02 | Superseding standard-ADF raw-track fix: generated ADF tracks now pad their trailing gap to a 12,668-byte DD raw-track revolution instead of ending at the shorter sector payload length. Focused proof: `mnemos_manifests_amiga_test.exe "[manifests][amiga500][disk]"` passed 493 assertions across 39 disk cases; `scripts/amiga/run-corpus-smoke.ps1 -BuildDir build\windows-msvc-release -BiosDir D:\emu\amiga\bios -Frames 4500 -MinimumHeadlessFps 50 -RequireDiskProgress -RejectKickstartPrompt -System amiga500 -Rom D:\emu\amiga\adf\Pang-500-NTSC.adf.zip` passed at 183.89 FPS, cylinder 73, 37,004 nonblack pixels, and 16 colors. Screenshot `build/scratch/amiga-corpus/amiga500-Pang-500-NTSC.ppm.png` shows a readable Pang cracktro/options screen, so A500 is promoted to visual smoke. The same route on A500+/A600 reaches readable `Software Failure` Guru prompts, so those routes move from visual defect to requirement/error prompt. The same fix also supersedes the earlier Rainbow Islands ADF disk-DMA failure: `scripts/amiga/run-corpus-smoke.ps1 -BuildDir build\windows-msvc-release -BiosDir D:\emu\amiga\bios -Frames 4500 -MinimumHeadlessFps 50 -RequireDiskProgress -RejectKickstartPrompt -System amiga500,amiga500plus,amiga600 -Rom D:\emu\amiga\adf\Rainbow-Islands.zip` now passes on all three models at 184+ FPS, reaches cylinder 25, and screenshots `build/scratch/amiga-corpus/amiga500-Rainbow-Islands.ppm.png`, `amiga500plus-Rainbow-Islands.ppm.png`, and `amiga600-Rainbow-Islands.ppm.png` show readable Rainbow Islands gameplay/attract content. |
| 2026-07-02 | Amiga archive-media selection was hardened so unsupported-size `.adf` members inside ZIP/TAR/tool-backed archives are ignored when the same archive also contains a supported ADF/ADZ/IPF floppy. Direct bad ADFs and archives with only unsupported-size ADF entries still fail with `media.amiga.adf.unsupported_size` / the existing launcher error. Regression proof: `mnemos_apps_player_system_launch_test.exe "[apps][player][launch][amiga500][zip]"` passed 219 assertions across 10 ZIP-backed A500 launch cases, and the full `mnemos_apps_player_system_launch_test.exe` passed 1114 assertions across 65 cases. A local trace launch of `D:\emu\amiga\adf\AlienSyndrome.zip` selected `Alien Syndrome.adf (901120 bytes)` from the archive. |
...
| 2026-07-02 | Superseded Rainbow Islands ADF failure analysis, retained for trace provenance. Before the standard-ADF raw-track padding fix above, Rainbow Islands remained `Broken (visual defect)` on A500/A500+/A600 after the WORDSYNC latch regression fix. The A500 trace (`build/scratch/rainbow-adf-wordsync-latch-trace300.stderr.txt`) showed the raw loader arming `DSKLEN=$9F40` with `DSKSYN` already latched and `wordsync=0`, so disk DMA started instead of waiting for another sync. The CPU still polled `DSKBLK` at `$0012AC`: the active Copper list reached `$00123A` and wrote `DMACON=$397C`, clearing `DSKEN` before the long raw read completed. Final board state from `build/scratch/rainbow-adf-wordsync-latch.ppm.amiga_board.regs.txt` was `INTREQ=0x1020` (`DSKSYN`/`VERTB` but no `DSKBLK`), `DSKPTR=0x0015B6`, `DSKLEN=0x9E68`, `DSKDMA=0x00003CD0`, `DFMOTOR=1`, and `COPRUN=1`; the screenshot was all black. No title-specific workaround was applied. |

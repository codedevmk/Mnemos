...
**Status:** In progress — the Z80 CPU, SN76489 PSG, SMS VDP, and Sega mapper have
landed, and `manifests::sms::assemble_sms` now wires them into a **bootable SMS**
(a unit test runs a cart program through reset → mapper banking → VDP/PSG I/O →
frame-IRQ ISR). The whole stack fit with **zero changes to i_cpu / i_bus / any
lower-tier contract**. Remaining: the headless CLI/scheduler run path, golden frame
tests (data-gated), Z80 ZEXALL conformance (data-gated), and the Codemasters mapper.
...
- [~] Implement SMS mapper (Sega mapper, Codemasters mapper). (chips::mapper::sms_mapper, ported from the Emu reference per ADR 0006 and improved: span-based borrowed ROM, std::array cart RAM, no debug-event coupling. The Sega mapper — three 16 KiB ROM slots (slot 0's first 1 KiB fixed to physical page 0), the optional 16 KiB on-cart RAM window at slot 2 with a bank-select bit, page wrap modulo the image page count, and the four control registers at $FFFC-$FFFF (fully decoded — the $DFFC-$DFFF RAM mirror is ignored). cpu_read/cpu_write for the $0000-$BFFF banked window + write_register for the register window; save/load (registers + cart RAM) + register_snapshot (CTRL/PAGE0-2); factory "sega.sms_mapper". 8 unit tests (identity+factory, default 0/1/2 paging, page switching with offset preserved, page wrap, cart RAM enable/bank/read-write, register-decode bounds, open bus with no ROM, state round-trip). Codemasters mapper deferred — absent from the Emu reference and unvalidatable without test ROMs; a follow-up)
- [x] Author `manifests/sms/` (NTSC + PAL). (manifests::sms::assemble_sms wires the Z80 + SMS VDP + SN76489 + Sega mapper into a bootable machine. Memory map: $0000-$BFFF banked ROM/cart-RAM via the mapper, $C000-$DFFF 8 KiB work RAM mirrored to $E000-$FFFF, with the $FFFC-$FFFF mapper-register overlay writing through to RAM. Z80 I/O routing: $00-$3F open-bus reads / $3F I/O-control latch writes, $40-$7F V/H counters (read) + PSG (write), $80-$BF VDP data/ctrl, $C0-$FF joypad ports $DC/$DD with the active-low pin + TH/TR nationalisation model. The VDP /INT line is ORed into the Z80 IRQ; SP is set to $DFF0 to emulate the post-BIOS hand-off. sms_config selects NTSC (262 lines) vs PAL (313). 7 system tests (boot Z80->RAM, region, mapper banking through the bus, VDP OUT, PSG OUT, joypad IN, and an end-to-end frame-IRQ ISR run). The headless CLI run path (scheduler wiring) is a follow-up)

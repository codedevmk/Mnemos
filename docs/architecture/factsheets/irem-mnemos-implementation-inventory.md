...
That scan found 439 local corpus items across 24 top-level buckets. Direct files
...
M102 now has a first-pass `hclimber` route: `hclimber.zip` is supported, and
`hclimber.7z` remains metadata-only until converted or unpacked. Red Alert now
has a first-pass `ww3` route: the unpacked
...
artwork/layout package remains metadata-only non-ROM proof.
...
Dream Soccer '94 Japan M92 hardware split set, while M10, Red Alert, M102, and
...
|---|---:|---:|---|---|---|---|
| M10 / M15 | `irem_m10` + `irem_m15` first-pass | 42% overall / 20% for M10 subset / 55% for Head On subset | `andromed`, `skychut`, `headoni` | `andromed` / `skychut` through native 8085 M10 route; `headoni` nonblank + save/load | None | Exact 8085 board timing, M10/M15 analog sound/sample mapping, analog color, exact raster phase, screenshot/audio parity |
| M14 | `irem_m14` first-pass | 18% | `ptrmj` | local P.T. Reach Mahjong ZIPs through native 8085 adapter; direct `mnemos_player --system irem_m14` nonblank screenshot and `--system m14` save-state proof | None | Deeper NEC D8085AC/8085 interrupt/timing proof, video/color, paddle/mahjong/input behavior, discrete/sample sound, visual/audio parity |
...
| M119 | `irem_m119` first-pass | 14% | `scumimon` | local `scumimon.zip` through the adapter; explicit SH7708/SH-3, uPD94244-210, and datasheet-shaped YMZ280B chip surfaces; SH-executed local-register/MMIO proof; YMZ280B 24-bit address, loop, PCM8/PCM16/ADPCM smoke tests; nonblank diagnostic frame, YMZ rendered-audio capture path, and save-state proof | None | Authentic SH-3 MMU/cache/timers/interrupts, uPD94244 raster/tile/sprite behavior, YMZ280B exact ADPCM edge cases, DSP/IRQ/bus timing, M119 board I/O/timing, DIP/mechanical behavior, and visual/audio parity |
...
- M10 now has Andromeda SS and Sky Chuter ROM contracts plus a first-pass native
  8085 player route. `MNEMOS_M10_SET_DIR=D:\emu\irem\M10` proves the local ZIPs
  through the adapter. Current graphics and audio remain diagnostic first-pass
  output, not board-authentic M10 parity.
- M14 now has a P.T. Reach Mahjong ROM contract plus a first-pass executable
  native 8085 route. `MNEMOS_M14_SET_DIR=D:\emu\irem\M14`
...
- M82 has scanline-composed tile/sprite/palette rendering with focused priority
  ...
  present.
- M10, M14, M15, M27, M47, M52, M57, M58, M62, M63, travrusa, Red Alert, M75,
  M78, M81, M84, M85, M90, M92, M102, and M107 all have player-routable first-pass boards with
  ...
  League-Man parent/alternate M92-Z-C routes, Superior/Perfect Soldiers
  parent/Japan split-clone routes, R-Type Leo parent/Japan split-wrapper
  routes, Dream Soccer '94 Japan on M92 hardware with explicit M107 supplemental
  shared media, and Undercover Cops parent/US/Japan/Alpha Renewal routes.
...
- **Techsheet games:** IPM Invader, Space Beam, Sky Chuter, Head On, Green Beret,
  Andromeda.
- **Mnemos games:** `andromed`, `skychut`, `headoni`.
- **Smoke playable:** `andromed` and `skychut` load through `--system irem_m10`
  / `--system m10` and pass the local `MNEMOS_M10_SET_DIR=D:\emu\irem\M10`
  corpus golden. Direct 120-frame player smokes wrote
  `build\scratch\irem-m10\andromed.ppm` and
  `build\scratch\irem-m10\skychut.ppm` at 224x256 with nonzero payload counts
  of 169087/172032 and 169397/172032 bytes respectively. `headoni` loads
  through `--system irem_m15`, produces a 224x256 nonblank frame, and has
  save/load smoke through the local M15 corpus.
- **Correct gfx/music:** not certified. Graphics are first-pass M15
  tile/color/chargen output plus M10 diagnostic GFX-ROM/RAM output; sound is
  board-owned latch/edge or beeper evidence, not final analog sample or music
  parity.
- **Remaining:** exact M10/M15 reset, memory, I/O, interrupt, and raster timing,
  board-evidenced discrete sound and analog color, and screenshot/audio parity.
...
- **Current implementation:** `src/manifests/irem_m14` embeds the local
  ...
  the native Intel 8085/8080 core configured as an 8085, program ROM at `$0000`,
  RAM, video, color, work, input/DIP/control MMIO, I/O-port mirroring for early
  CPU code, a GFX-ROM-driven diagnostic compositor, a beeper-backed sound latch,
  save-state identity, and player registration under `irem_m14` / `m14`.
- **Remaining:** prove deeper NEC D8085AC/8085 interrupt/timing behavior and the
  real M14 memory and I/O map, raster/color behavior, paddle/mahjong/input
  behavior, sparse discrete or sample sound, and visual/audio parity.
...
- **Techsheet games:** Gunforce, Lethal Thunder / Thunder Blaster, R-Type Leo,
  In the Hunt, Undercover Cops, Dream Soccer '94 Japan M92 hardware variant,
...
- **Mnemos games:** `bmaster`, `crossbld`, `dsoccr94j`, `geostorm`, `geostorma`,
  ...
  `majtitl2j`, `mysticri`, `mysticrib`, `nbbatman`, `nbbatmanu`,
  `psoldier`, `rtypeleo`, `rtypeleoj`, `ssoldier`, `thndblst`, `uccops`,
  `uccopsar`, `uccopsj`, and `uccopsu` as checked-in manifests under
  `irem_m92`.
- **Smoke playable:** all thirty-six checked-in M92 manifests load through
  ...
  `uccopsj.zip` and `uccopsar.zip` have direct save-state proof. `majtitl2.zip`
  ...
  and 320x240 nonblank PPM proof. `ssoldier.zip` and split-clone
  `psoldier.zip` have direct save-state, load-state, and 320x240 nonblank PPM
  proof through the local `ssoldier` parent fallback.
  `dsoccr94j.zip` has direct `mnemos_player --system irem_m92` proof when
  launched with supplemental `D:\emu\irem\M107\dsoccr94.zip` media: 320x240
  nonblank PPMs after 60 frames and save-state/load-state bytes.
...
- **Local corpus note:** the current `D:\emu\irem\M92` bucket has 82 tracked
  M92 manifest-name matches, 37 direct player-loadable/supported ZIP routes,
  and 45 metadata-only routes, including `.7z` archives and artwork/layout
  packages. Thirty-six checked-in M92-era set IDs now resolve to embedded
  manifests and load CRC-clean through `MNEMOS_M92_SET_DIR`, with
  `D:\emu\irem\M107` included as supplemental media for `dsoccr94j`: Blade Master
  parent/Japan clone (`bmaster`, `crossbld`), Gunforce parent
  (`gunforce`), Gunforce Japan/US split clones (`gunforcej`, `gunforceu`) via
  parent fallback, Gunforce 2 / Geostorm (`gunforc2`, `geostorm`,
  `geostorma`) via parent fallback, Hook parent/Japan clone (`hook`, `hookj`),
  In the Hunt parent/US/Japan/Korea clones (`inthunt`, `inthuntu`,
  `kaiteids`, `inthuntk`) via parent fallback, Lethal Thunder
  parent/Japan clone (`lethalth`, `thndblst`) via parent fallback, Mystic Riders
  (`mysticri`), Gun Hohki (`gunhohki`), Major Title 2 parent,
  ...
  fallback, Superior Soldiers parent / Perfect Soldiers Japan split clone
  (`ssoldier`, `psoldier`) via parent fallback, R-Type Leo parent/Japan clone
  (`rtypeleo`, `rtypeleoj`) via parent fallback, Dream Soccer '94 Japan M92
  hardware (`dsoccr94j`) with explicit `dsoccr94.zip` supplemental shared media,
  and Undercover Cops
...
- **Mnemos games:** `hclimber` as a checked-in ROM contract plus first-pass
  player route under `src/manifests/irem_m102` and
  `src/apps/player/adapters/irem_m102`.
- **Smoke playable:** `MNEMOS_M102_SET_DIR=D:\emu\irem\M102` data-gates the
  local `hclimber.zip` route and proves the program ROM, GA20 sample ROMs, and
  explicit zero-filled no-dump PAL placeholder region load CRC-clean through the
  adapter. Direct `mnemos_player --system irem_m102 --rom
  D:\emu\irem\M102\hclimber.zip` smoke produces a 320x240 nonblank diagnostic
  frame and save-state bytes.
- **Correct gfx/music:** not certified. Current graphics are diagnostic
  ROM/RAM-derived output, and GA20 capture proves only the first-pass sample
  path, not authentic Hill Climber sound behavior.
- **Local corpus note:** `D:\emu\irem\M102` holds `hclimber.zip` and
  `hclimber.7z`; the ZIP is a tracked player-supported route and the `.7z`
  archive remains metadata-only until converted or unpacked. The M102 contract
  also asserts the public note that the upper half of `hc-pr-c.ic23` is blank
  (`0xff`) rather than executable code.
- **Remaining:** exact NEC D70008AC/Z80 timing, exact memory/I/O and two DIP
  banks, electromechanical medal/connector I/O, artwork/mechanical model,
  title-specific video timing, and visual/audio parity before any Hill Climber
  correctness claim.
...
- **Current implementation:** the SH7708 shell now exposes serializable local
  ...
  the first uPD94244 registers as big-endian 32-bit MMIO lanes plus the
  documented YMZ280B channel/utility byte-register map. The YMZ path covers
  24-bit start/loop/end addresses, PCM8, MSB-first PCM16, ADPCM nibble decode,
  loop playback, and enabled end-status latching. The `scumimon` route has
  direct player smoke proof with a nonblank 512x384 diagnostic PPM and rendered
  YMZ audio output.
- **Remaining:** replace the SH-2-compatible SH-3 execution bridge with
  ...
  rendering, prove YMZ280B exact ADPCM/DSP/IRQ/bus timing, prove board
...
8. Advance M14 P.T. Reach Mahjong from first-pass native-8085 smoke-playable to
   authentic behavior by proving board-evidenced 8085 timing, input, color, and
   sound behavior.

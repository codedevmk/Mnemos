...
| M62 | `irem_m62` first-pass Lode Runner/Lot Lot MC6803/SSG/MSM5205 route plus raw-media fallback | 27% | `battroad`, `horizon`, `ldrun`, `ldruna`, `ldrun2`, `ldrun3`, `ldrun3j`, `ldrun4`, `lotlot`, `spelunk2`, `youjyudn` | all 11 local ZIP routes through the adapter; `ldrun`, `ldrun2`, `ldrun3`, `ldrun4`, and `lotlot` region contracts load Z80 program, MC6803 sound ROM, graphics, PROM, and timing regions with the MC6803 reset vector proven in the `$8000-$ffff` sound-ROM window; direct `mnemos_player --system irem_m62` nonblank screenshot/save-state proof for `ldrun`, `ldrun2`, `ldrun3`, `ldrun4`, and `lotlot` | None | Exact M62 title bus maps, MC6803 port/timer timing, exact dual MSM5205 stream/control timing, KNA custom video, inputs/DIPs, visual/audio parity |
...
- M62 now has eleven ROM-set contracts plus a first-pass player route with
  ...
  save-state proof. `ldrun`, `ldrun2`, and `ldrun3` have been promoted from
  raw-media-only staging to explicit Z80 program, MC6803 sound ROM, graphics,
  PROM, and timing regions, and the board route wires a first-pass MC6803
  direct-page latch/dual-SSG path for the Lode Runner family.
...
- **Mnemos games:** explicit `ldrun`, `ldrun2`, `ldrun3`, `ldrun4`, and `lotlot` region contracts plus
...
- **Current implementation:** `src/manifests/irem_m62` embeds local ROM-set
  ...
  Horizon, and Youjyuden artifacts. `ldrun`, `ldrun2`, `ldrun3`, `ldrun4`, and `lotlot` now map
  ...
  grouping are preserved without asserting unfinished title-specific bus
  placement.
  ...
  regioned Lode Runner sets. The player adapter exposes Z80, MC6803, both SSG,

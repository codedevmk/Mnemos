...
> **Status (2026-05-29, latest):** **Phase B COMPLETE — all three systems (SMS, Genesis, C64) build through `build_system`.** B.3 (C64) landed: `build_c64_runtime` == `assemble_c64` byte-for-byte (synthetic + real BASIC/KERNAL/CHARGEN boot, 30 frames; passed first run). The C64's PLA dynamic banking required two foundation extensions — per-region `active_predicate` + explicit priority (regions/chip-MMIO name a host `bool(addr,is_write)` predicate; the C64 PLA decode reads the live 6510 `$01` port + cart lines), and `system_graph::region_span` (the VIC shares the RAM/CHARGEN/colour buffers build_system allocated). Also fixed the pre-existing c64 manifest's wrong VIC factory id (`mos.6567`→`6569`) and placeholder sha256. No C64 player adapter exists, so no cutover — the parity test is the deliverable. Each `assemble_*()` is retained as its system's oracle. Commits `3ae6df4`, `73c0801`, `f4d15f5`, `ba0563a`. **Remaining: Phase C cleanup (optional, after soak).**

> **Status (2026-05-29, later):** **Phase B.2 COMPLETE — Genesis adapter fully cut over to the manifest path.** `genesis_adapter` now builds via `build_genesis_runtime()` (embedded manifest + `build_system` + `genesis_callbacks`); `assemble_genesis` is retained UNCHANGED as the byte-for-byte parity oracle. Added: `genesis.ntsc/pal.toml`, the `genesis_callbacks` bundle (callbacks + predicates + 11 MMIO factories across two buses), `genesis_runtime` (resolves the gated 68K/Z80 scheduler wrappers), embedded manifests, and `genesis_manifest_parity_test`. **Two foundation fixes surfaced:** (1) the manifest parser wrongly *required* `sha256` on every rom region — now optional (`file` required), so runtime carts work; (2) `build_genesis_runtime` resets the CPUs after wiring because `build_system` never resets chips and the 68000 ctor can't load vectors with no bus — without it the 68000 ran from PC=0 (found via a PC-trace diagnostic: divergence at instruction 0). Verified: `build_genesis_runtime` == `assemble_genesis` byte-for-byte on Aerobiz (PC-identical 87482 instructions; framebuffer 0/diff at 30 frames); full suite 52 entries, 0 failures; SMS + SHA-pinned `sms_boot` unaffected. Commits `2621f62`, `3cc15f6`, `9d2077a`, `7a17ac3`. **Next: B.3 (C64), then Phase C cleanup.**
...
### B.2 — Genesis migration ✅ COMPLETE (2026-05-29)

`genesis_adapter` is cut over to `build_genesis_runtime()` (commit `7a17ac3`); `assemble_genesis` is retained UNCHANGED as the parity oracle. Rather than re-running the the reference 6-title harness, the migration gate is **manifest-path == `assemble_genesis` byte-for-byte** (`genesis_manifest_parity_test`): since the oracle is untouched, its the reference byte-perfect status is preserved by construction. Verified PC-identical (87482 instructions) and framebuffer-identical on Aerobiz. Two foundation fixes landed (parser: `sha256` optional on rom regions; `build_genesis_runtime` resets the CPUs post-wiring since `build_system` doesn't). The original "thin-wrapper / 6-title the reference gate" plan below is superseded by the oracle-parity approach proven on SMS.
...
### B.3 — C64 migration ✅ COMPLETE (2026-05-29)

`build_c64_runtime` == `assemble_c64` byte-for-byte (commit `ba0563a`). The flagged PLA-banking risk was real and drove two `build_system` foundation extensions (per-region `active_predicate` + priority; `region_span` for the VIC's shared buffers). `assemble_c64` retained as the oracle; no player adapter, so no cutover.

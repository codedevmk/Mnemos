...
`genesis_adapter` is cut over to `build_genesis_runtime()` (commit `7a17ac3`); `assemble_genesis` is retained UNCHANGED as the parity oracle. Rather than re-running the reference 6-title harness, the migration gate is **manifest-path == `assemble_genesis` byte-for-byte** (`genesis_manifest_parity_test`): since the oracle is untouched, its byte-perfect status is preserved by construction. Verified PC-identical (87482 instructions) and framebuffer-identical on Aerobiz. Two foundation fixes landed (parser: `sha256` optional on rom regions; `build_genesis_runtime` resets the CPUs post-wiring since `build_system` doesn't). The original "thin-wrapper / 6-title reference gate" plan below is superseded by the oracle-parity approach proven on SMS.

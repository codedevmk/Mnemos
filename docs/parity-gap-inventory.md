...
V33 latch write through V35 latch read, sound-RAM storage, reply-port write,
save-state persistence, the shared Fire Barrel / Air Assault SW1/SW2 DIP
profile default (`0xffbf`), and the separate SW3 `COINS_DSW3` default
(`0xebff`). Root and `m72`-folder duplicates are also visible as M107 artifacts;
the two Fire Barrel `.7z` copies remain metadata-only until converted or
unpacked.

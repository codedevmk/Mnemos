...
> gzip-wrapped ADF (`.adz` / `.adf.gz`), ZIP-contained ADF,
...
> Turrican II and Xenon II wrapper smoke proof, plus a `.7z` ADF fixture smoke
> proof across A500/A500+/A600. Separate `--rom`/`--disk` media
...
> in-flight busy countdown save states. Paula buffer wraps, CPU-fed AUDxDAT
> manual playback, and ADKCON volume/period attachment now route through the
> Amiga audio path: AUD0-AUD3 requests reach INTREQ/level-4 CPU interrupt
> gating, attached modulator channels suppress their DAC output, and INTREQ
> acknowledgement clears Paula's source latch.

...
player support: 115 items match a checked-in Irem manifest, 106 are readable
...
manifests reports `1216/1216` required files present from
...
direct player-loadable clone of `vigilant`, reducing the current
`classify_or_sort_corpus_item` root rows from 15 to 14.

...
> full Mode-4 *content* rendering and the GPGX M5-down-toggle stale-height latch
> remain. (Methodology note: the pinned `results_on.txt` baseline is weeks stale;
> isolating a change now requires a same-tree current-HEAD baseline player, not
> that file.)

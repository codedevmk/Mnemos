...
#### Manifests / board bring-up
- [x] **I75-1** Vigilante ROM-set contract — `src/manifests/irem_m75` carries a checked-in embedded ROM-contract manifest for `vigilant`, including the Z80 main program with banked `$8000-$bfff` reload area, sound CPU ROM, character graphics, sprite graphics with ROM-continue duplication, background tile graphics, samples, PROM, and PLD regions. `MNEMOS_M75_SET_DIR=D:\emu\irem` data-gates the complete local parent wrapper `D:\emu\irem\Vigilante_Arcade_EN (3).zip`, which unwraps to `vigilant.zip` and loads CRC-clean; `scripts/irem/inventory-corpus.ps1` now records one direct player-loadable M75 manifest match instead of leaving Vigilante as an unsupported root item · DONE · MED · S · beyond Emu · Evidence: `src/manifests/irem_m75/games/vigilant.toml` + `src/manifests/irem_m75/tests/m75_rom_contract_test.cpp` + `scripts/irem/inventory-corpus.ps1`
- [~] **I75-2** Executable M75 board profile — `src/manifests/irem_m75/m75_system.cpp` now assembles a first-pass M75-owned Z80/Z80/YM2151/DAC board with fixed and banked main ROM windows, sprite/palette/video/work RAM, sound ROM/RAM, active-low inputs/DIPs, Vigilante bank/scroll/rear-color latches, sound latch IRQ/ack, sample-address and DAC ports, a two-bank 5-bit KNA91-style palette bus, rear color/disable register semantics, whole-board save/load identity, and diagnostic video composition over the checked-in graphics/sample/PROM regions. `src/apps/player/adapters/irem_m75` registers `--system irem_m75` / `m75`, supports direct ZIPs, single-inner wrapper ZIPs, unpacked folders, embedded or in-archive `game.toml` manifests, resident media validation, rollback-ready save-state, capability discovery, and real local player smoke through `MNEMOS_M75_SET_DIR=D:\emu\irem`. Remaining: replace the diagnostic compositor with board-evidenced Vigilante tile/sprite/background priority, verify exact memory/I/O/DIP/raster timing, prove sample/DAC behavior against reference evidence, add clone/bootleg parent-fallback coverage for partial regional ZIPs, and collect screenshot/audio parity before calling Vigilante correct · PARTIAL · HIGH · M-L · beyond Emu · Evidence: `src/manifests/irem_m75/m75_system.cpp` + `src/manifests/irem_m75/tests/m75_system_test.cpp` + `src/apps/player/adapters/irem_m75/irem_m75_adapter.cpp` + `src/apps/player/adapters/irem_m75/tests/irem_m75_adapter_test.cpp` + `docs/architecture/factsheets/irem-system-boards-reference.md`

...
Only games the Mnemos CPS1 board can actually run are emitted. The QSound +
Kabuki sound titles (dino / punisher / wof) ARE emitted now -- they carry a
`qsound` sample region plus `[set] sound = "qsound"` and `kabuki = "<game>"`
keys the board reads. Titles whose sound decode the board does not yet wire,
and timekeeper-RTC titles, are still skipped. A game is also skipped if its
local zip is missing or lacks a named file, so every emitted set is fully
CRC-verifiable.
...
# Emu region enum -> Mnemos romset region name. Regions absent here (e.g.
...
REGION_TOML = {
    ...
    "CPS1_ZIP_REGION_QSOUND": "qsound",
    "CPS1_ZIP_REGION_GFX": "gfx",
}
REGION_ORDER = ["maincpu", "gfx", "audiocpu", "oki", "qsound"]

# Emu sound-decode enum -> the board's (sound_system, kabuki) [set] keys. A
# decode absent here is a QSound variant the board does not yet wire -> skip.
SOUND_DECODE = {
    "CPS1_SOUND_DECODE_NONE": (None, None),
    "CPS1_SOUND_DECODE_KABUKI_DINO": ("qsound", "dino"),
    "CPS1_SOUND_DECODE_KABUKI_PUNISHER": ("qsound", "punisher"),
    "CPS1_SOUND_DECODE_KABUKI_WOF": ("qsound", "wof"),
}
...
def emit_toml(short_name, desc, profile_id, vertical, sound, kabuki, region_sizes, region_parts,
              crcs):
    lines = []
...
for d in descriptors:
    ...
    if sound_decode not in SOUND_DECODE:
        skipped.append((short_name, f"unsupported sound decode {sound_decode}"))
        continue
    sound, kabuki = SOUND_DECODE[sound_decode]
    if not parts:
        skipped.append((short_name, "no parts table"))
        continue
    if any(p["region"] not in REGION_TOML for p in parts):
        skipped.append((short_name, "needs an unsupported region (timekeeper)"))
        continue
    ...
    text = emit_toml(short_name, desc, profile_id, vertical, sound, kabuki, sizes, region_parts,
                     crcs)

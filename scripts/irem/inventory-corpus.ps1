...
function Get-BoardCandidateFamily {
    ...
    if ($Bucket -in @("M15", "M72", "M81", "M82", "M84", "M90", "M92", "M107", "i8751")) {
        return $Bucket
    }
    return ""
}
...
function New-ArchiveItem {
    ...
    $trackedMatch = $m15Match -or $m72Match -or $m81Match -or $m82Match -or $m84Match -or $m90Match -or $m92Match -or $m107Match
    ...
    $trackedFamily = Get-TrackedFamilyName -M15Match $m15Match -M72Match $m72Match -M81Match $m81Match -M82Match $m82Match -M84Match $m84Match -M90Match $m90Match -M92Match $m92Match -M107Match $m107Match
...
foreach ($root in @($roots)) {
    ...
    if (Test-Path -LiteralPath $rootPath -PathType Leaf) {
        ...
        $items.Add((New-ArchiveItem -File $file -Bucket $bucket -M15ManifestIds $m15ManifestIds -M72ManifestIds $m72ManifestIds -M81ManifestIds $m81ManifestIds -M82ManifestIds $m82ManifestIds -M84ManifestIds $m84ManifestIds -M90ManifestIds $m90ManifestIds -M92ManifestIds $m92ManifestIds -M107ManifestIds $m107ManifestIds))
        continue
    }
    ...
    foreach ($directory in Get-ChildItem @dirArgs | Sort-Object FullName) {
        ...
        $item = New-DirectoryItem -Directory $directory -Bucket $bucket -M15ManifestIds $m15ManifestIds -M72ManifestIds $m72ManifestIds -M81ManifestIds $m81ManifestIds -M82ManifestIds $m82ManifestIds -M84ManifestIds $m84ManifestIds -M90ManifestIds $m90ManifestIds -M92ManifestIds $m92ManifestIds -M107ManifestIds $m107ManifestIds
        if ($null -ne $item) {
            $items.Add($item)
        }
    }
...
$report = [pscustomobject]@{
    ...
    known_buckets = @("M15", "M72", "M81", "M82", "M84", "M90", "M92", "M107", "i8751", "root")
...
foreach ($bucket in $bucketRows) {
    Write-Host ("  [{0}] items={1} archives={2} dirs={3} m15_matches={4} m72_matches={5} m81_matches={6} m82_matches={7} m84_matches={8} m90_matches={9} m92_matches={10} m107_matches={11} tracked={12} loadable={13} supported={14} metadata_only={15}" -f $bucket.bucket, $bucket.item_count, $bucket.archive_count, $bucket.directory_count, $bucket.m15_manifest_matches, $bucket.m72_manifest_matches, $bucket.m81_manifest_matches, $bucket.m82_manifest_matches, $bucket.m84_manifest_matches, $bucket.m90_manifest_matches, $bucket.m92_manifest_matches, $bucket.m107_manifest_matches, $bucket.tracked_by_mnemos, $bucket.loadable_by_mnemos, $bucket.supported_by_mnemos, $bucket.metadata_only_tracked)
}

...
`src/instrumentation/wire/`; <!-- nolint: deliberately historical --> the debugger/wire tooling moved to `src/debug/`
...
(`tools/schema_compat.py`, gate G8) <!-- nolint: P2 tool --> lands in pilot
phase P2 alongside the first schemas.
...
- A `mnemos-py` package, versioned with the wire schema, provides idiomatic <!-- nolint: future package -->
  bindings.

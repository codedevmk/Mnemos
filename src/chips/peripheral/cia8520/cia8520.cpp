// Ported from the Emu reference (chips/cia8520); clean-room per the MOS 8520 datasheet.
...
namespace mnemos::chips::peripheral {
    ...
    void cia8520::reset(reset_kind /*kind*/) {
        pra_out_ = 0U;
        prb_out_ = 0U;
        ddra_ = 0U;
        ddrb_ = 0U;
        ...
        tod_.divider_reload = tod_.src_hz != 0U ? cfg_.tod_tick_hz / tod_.src_hz : 0U;
    ...
    void cia8520::icr_raise(std::uint8_t bits) {
    ...
    std::uint8_t cia8520::port_b_driven() const noexcept {
        auto v = prb_out_;
    ...
    std::uint8_t cia8520::port_a_pins() const {
        const std::uint8_t pins = cfg_.read_port_a ? cfg_.read_port_a() : 0xFFU;
        return static_cast<std::uint8_t>((pra_out_ & ddra_) |
                                         (pins & static_cast<std::uint8_t>(~ddra_)));
    }

    std::uint8_t cia8520::port_a_output() const noexcept {
        return static_cast<std::uint8_t>(pra_out_ & ddra_);
    }

    std::uint8_t cia8520::port_b_output() const noexcept {
        return static_cast<std::uint8_t>(prb_out_ & ddrb_);
    }

    std::uint8_t cia8520::read_pb_live() {
    ...
    void cia8520::timer_handle_underflow(timer_state& t, bool is_timer_a) {
        t.underflow = true;
        t.counter = t.latch;
    ...
    void cia8520::timer_step(timer_state& t, unsigned count, bool is_timer_a) {
        ...
        while (count-- > 0U) {
            ...
            // continuous-mode period is latch+1.
            if (t.counter == 0U) {
    ...
    void cia8520::sdr_step_output_bit() {
        if (!sdr_.shifting) {
    ...
    void cia8520::sdr_step_input_bit() {
        sdr_.shift_reg =
    ...
    std::uint8_t cia8520::read(std::uint8_t address) {
    ...
    void cia8520::write(std::uint8_t address, std::uint8_t value) {
        switch (address & 0x0FU) {
        case reg_pra:
            if (pra_out_ != value) {
                pra_out_ = value;
    ...
    void cia8520::tick(std::uint64_t cycles) {
        for (std::uint64_t i = 0; i < cycles; ++i) {
            irq_pin_update(); // 1-clock delayed publication of the IR flip-flop
            ...
            if (timer_a_.underflow && sdr_.output_mode) {
                // The serial shifter is clocked by an internal half-rate flip-flop
                // on TA underflow, independent of CRA.PBON.
    ...
    void cia8520::flag_edge() { icr_raise(icr_flag); }

    void cia8520::cnt_edge(bool new_level) {
    ...
    void cia8520::sp_level(bool new_level) { sdr_.sp_level = new_level; }

    std::uint8_t cia8520::pb_timer_bits() const noexcept {
        std::uint8_t v = 0U;
        if ((timer_a_.cr & cra_pbon) != 0U) {
            const bool level =
                (timer_a_.cr & cra_outmode) != 0U ? timer_a_.toggle_pb : timer_a_.pulsing_pb;
    ...
    void cia8520::save_state(state_writer& writer) const {
    ...
    void cia8520::load_state(state_reader& reader) {

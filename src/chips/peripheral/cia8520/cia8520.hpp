...
namespace mnemos::chips::peripheral {
    ...
    //
    // The 8520 shares the MOS 6526 silicon surface: two 8-bit I/O ports with DDR,
    // two 16-bit interval timers with cascade/pin-output and the silicon-accurate
    // force-load + start-delay pipelines, a BCD time-of-day clock with alarm, an
    // 8-bit serial shift register with SP/CNT pins, and the NMOS edge-triggered
    // interrupt control with a 1-clock /IRQ propagation delay. Relative to the
    // 6526 it differs only in the external clock-edge convention (/E clock rather
    // than PHI2) and the HMOS process rework; the register layout, timer, TOD,
    // serial, and IRQ-latch logic that a register-accurate model cares about are
    // identical, so this peripheral implements the same state machine.
    //
    // tick() advances one source clock; all time-domain behaviour evolves on tick,
    // never on register access.
    class cia8520 final : public iperipheral, public immio {
      public:
        ...
        // 16-register MMIO window (mirrored every 16 registers).
        ...
        // DDRA), input bits from the read callback or pulled high.

...
#ifndef MNEMOS_IREM_M27_GAMES_DIR
#define MNEMOS_IREM_M27_GAMES_DIR ""
#endif
...
namespace {
    ...
    [[nodiscard]] std::set<std::string, std::less<>> embedded_set_names() {
        ...
        for (const auto& [set_name, _] : mnemos::manifests::irem_m27::embedded::game_manifests) {
    ...
    [[nodiscard]] rom_set_decl require_embedded_decl(std::string_view set_name) {
        const std::string_view text = mnemos::manifests::irem_m27::game_manifest_toml(set_name);
...
TEST_CASE("m27 embedded manifests cover local ROM contracts", "[m27][romset]") {
    const auto expected_sets = embedded_set_names();
    CHECK(expected_sets == std::set<std::string, std::less<>>{"panther"});

    for (const auto& [set_name, toml] : mnemos::manifests::irem_m27::embedded::game_manifests) {
        INFO("set=" << set_name);
        auto parsed =
            mnemos::manifests::common::parse_rom_set_decl(toml, "embedded:irem_m27/" +
                                                                    std::string{set_name});
        for (const auto& error : parsed.errors) {
            INFO(error.source << ":" << error.line << ":" << error.column << ": " << error.message);
        }
        ...
        CHECK(decl.board == "irem_m27");
        CHECK(decl.orientation == mnemos::manifests::common::screen_orientation::horizontal);
        CHECK_FALSE(decl.parent.has_value());
        REQUIRE(decl.regions.size() == expected_panther_regions().size());

        for (const auto& [region_name, expected] : expected_panther_regions()) {
...
TEST_CASE("m27 embedded manifests stay in sync with disk TOML", "[m27][romset]") {
    const std::filesystem::path games_dir{MNEMOS_IREM_M27_GAMES_DIR};
    ...
    for (const std::filesystem::directory_entry& entry :
        ...
        CHECK(normalized_text(std::string{
                  mnemos::manifests::irem_m27::game_manifest_toml(set_name)}) ==
        ...
        CHECK(parsed.value->board == "irem_m27");
...
TEST_CASE("m27 local wrapper artifacts load CRC-clean through embedded manifests",
          "[m27][romset][data]") {
    const auto dir_env = environment_value("MNEMOS_M27_SET_DIR");
    if (!dir_env.has_value() || dir_env->empty()) {
        SKIP("set MNEMOS_M27_SET_DIR to directories containing the M27 wrapper corpus");
    }

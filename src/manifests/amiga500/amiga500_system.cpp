...
namespace mnemos::manifests::amiga500 {
    ...
    namespace {
        constexpr std::uint32_t state_version = 16U;
        ...
        [[nodiscard]] std::uint32_t merge_ptr(std::uint32_t old, bool high,
            ...
            if (high) {
                return ((static_cast<std::uint32_t>(value) & 0x001FU) << 16U) | (old & 0x0000FFFEU);
            }
            return (old & 0x001F0000U) | (static_cast<std::uint32_t>(value) & 0x0000FFFEU);
        }
        ...
        [[nodiscard]] std::uint16_t shifted_word(std::uint16_t previous, std::uint16_t current,
            ...
            const std::uint32_t composite =
                (static_cast<std::uint32_t>(previous) << 16U) | current;
        ...
        [[nodiscard]] std::int32_t blitter_line_error(std::uint32_t apt,
        ...
        [[nodiscard]] std::uint32_t step_blitter_pointer(std::uint32_t pointer,
            ...
            return static_cast<std::uint32_t>(
                       static_cast<std::int32_t>(pointer & 0x001FFFFEU) + delta) &
                   0x001FFFFEU;
        }
        ...
        [[nodiscard]] std::uint32_t merge_disk_ptr(std::uint32_t old, bool high,
                                                   std::uint16_t value) noexcept {
            if (high) {
                return ((static_cast<std::uint32_t>(value) & 0x0007U) << 16U) | (old & 0x0000FFFEU);
            }
            return (old & 0x00070000U) | (static_cast<std::uint32_t>(value) & 0x0000FFFEU);
        }
        ...
        [[nodiscard]] std::uint8_t pot_axis_value(std::uint8_t resistance,
            ...
            const std::uint64_t target =
                (static_cast<std::uint64_t>(resistance) * pot_full_scale_scanlines + 127U) /
                255U;
        ...
        [[nodiscard]] bool decode_amigados_sector(std::span<const std::uint8_t> track_bytes,
                                                  std::size_t offset,
                                                  std::uint8_t expected_track,
                                                  std::uint8_t& sector,
                                                  std::array<std::uint8_t,
                                                             amiga500_system::floppy_sector_size>&
                                                      sector_data) noexcept {
            ...
            constexpr std::size_t data_encoded_longs =
                amiga500_system::floppy_sector_size / 2U;
            constexpr std::size_t sector_bytes =
                sync_bytes + header_longs * 4U + checksum_bytes * 2U +
            ...
            const std::uint32_t stored_header_checksum =
                mfm_decode_odd_even(read_be32(track_bytes, cursor),
                                    read_be32(track_bytes, cursor + 4U));
            cursor += checksum_bytes;
            if (stored_header_checksum != mfm_checksum(header_encoded)) {
                return false;
            }

            const std::uint32_t stored_data_checksum =
                mfm_decode_odd_even(read_be32(track_bytes, cursor),
                                    read_be32(track_bytes, cursor + 4U));
            cursor += checksum_bytes;

            const std::uint32_t info =
                mfm_decode_odd_even(header_encoded[0], header_encoded[1]);
            ...
            if (sector >= amiga500_system::floppy_sectors_per_track ||
                sectors_remaining !=
                    static_cast<std::uint8_t>(amiga500_system::floppy_sectors_per_track -
                                              sector)) {
                return false;
            }
    ...
    std::uint16_t amiga500_system::read_custom_word(std::uint16_t reg) noexcept {
        ...
        case reg_dskbytr: {
            ...
            const bool dma_on = disk_dma_bytes_remaining != 0U &&
                                (disk_length & 0x8000U) != 0U && agnus.dma_disk() &&
                                drive != nullptr && !drive->image.empty() && floppy_selected &&
                                floppy_motor_on;
            if (disk_byte_valid) {
        ...
        case reg_intreqr:
            return intreq;
        case reg_dskpth:
            return static_cast<std::uint16_t>((disk_pointer >> 16U) & 0x0007U);
        case reg_dskptl:
        ...
        case reg_bltalwm:
            return bltalwm;
        case reg_bltcpt:
            return static_cast<std::uint16_t>((blitter_pointer[blit_c] >> 16U) & 0x001FU);
        case reg_bltcpt + 2U:
            return static_cast<std::uint16_t>(blitter_pointer[blit_c] & 0xFFFEU);
        case reg_bltbpt:
            return static_cast<std::uint16_t>((blitter_pointer[blit_b] >> 16U) & 0x001FU);
        case reg_bltbpt + 2U:
            return static_cast<std::uint16_t>(blitter_pointer[blit_b] & 0xFFFEU);
        case reg_bltapt:
            return static_cast<std::uint16_t>((blitter_pointer[blit_a] >> 16U) & 0x001FU);
        case reg_bltapt + 2U:
            return static_cast<std::uint16_t>(blitter_pointer[blit_a] & 0xFFFEU);
        case reg_bltdpt:
            return static_cast<std::uint16_t>((blitter_pointer[blit_d] >> 16U) & 0x001FU);
        case reg_bltdpt + 2U:
    ...
    void amiga500_system::write_custom_word(std::uint16_t reg, std::uint16_t value) noexcept {
        ...
        if (reg >= reg_bplpt_base &&
            ...
            bitplane_pointer[plane] = merge_ptr(bitplane_pointer[plane], high, value);
            agnus.set_bitplane_pointer(plane, bitplane_pointer[plane]);
            return;
        }
        ...
        case reg_copcon:
            ...
            return;
        case reg_cop1lch:
            cop1lc = merge_ptr(cop1lc, true, value);
            agnus.write_cop1lc(cop1lc);
            return;
        case reg_cop1lcl:
            cop1lc = merge_ptr(cop1lc, false, value);
            agnus.write_cop1lc(cop1lc);
            return;
        case reg_cop2lch:
            cop2lc = merge_ptr(cop2lc, true, value);
            agnus.write_cop2lc(cop2lc);
            return;
        case reg_cop2lcl:
            cop2lc = merge_ptr(cop2lc, false, value);
            agnus.write_cop2lc(cop2lc);
            return;
        case reg_copjmp1:
    ...
    void amiga500_system::write_custom_byte(std::uint32_t address, std::uint8_t value) noexcept {
        const std::uint16_t reg = static_cast<std::uint16_t>((address - custom_base) & 0x01FEU);
        if ((address & 1U) == 0U) {
            custom_high_latch = static_cast<std::uint16_t>(value << 8U);
            return;
        }
        write_custom_word(reg, static_cast<std::uint16_t>(custom_high_latch | value));
    }
    ...
    void amiga500_system::unmount_floppy() noexcept {
        unmount_floppy(0U);
    }
    ...
    bool amiga500_system::enqueue_keyboard_key(std::uint8_t raw_keycode, bool pressed) noexcept {
        return enqueue_keyboard_code(
            static_cast<std::uint8_t>((raw_keycode & 0x7FU) | (pressed ? 0x00U : 0x80U)));
    }
    ...
    bool amiga500_system::press_caps_lock() noexcept {
        ...
        const std::uint8_t code =
            static_cast<std::uint8_t>(0x62U | (next_led ? 0x00U : 0x80U));
    ...
    std::uint8_t amiga500_system::cia_a_port_a_inputs() const noexcept {
        ...
        if (floppy_selected) {
            const auto* drive = active_floppy_drive_state();
            const bool loaded = drive != nullptr && !drive->image.empty();
    ...
    void amiga500_system::update_floppy_track_stream(std::size_t drive) {
        ...
        for (std::uint8_t sector = 0U; sector < floppy_sectors_per_track; ++sector) {
            ...
            append_amigados_sector(df.track_stream,
                                   std::span<const std::uint8_t>(df.image)
                                       .subspan(sector_base, floppy_sector_size),
                                   track, sector);
        }
    ...
    bool amiga500_system::flush_floppy_track_to_image(std::size_t drive) noexcept {
        ...
        const std::size_t track_index =
            (static_cast<std::size_t>(df.cylinder_pos) * floppy_heads) + floppy_side_pos;
        if (track_index >= floppy_cylinders * floppy_heads) {
            return false;
        }
        ...
        for (std::size_t offset = 0U; offset + 4U <= df.track_stream.size(); ++offset) {
            if (read_be32(std::span<const std::uint8_t>(df.track_stream), offset) !=
                0x44894489U) {
                continue;
            }
    ...
    std::uint8_t amiga500_system::next_floppy_byte() noexcept {
        ...
        const std::uint8_t value = drive->track_stream[drive->stream_offset];
        drive->stream_offset = (drive->stream_offset + 1U) % drive->track_stream.size();
        disk_data = static_cast<std::uint16_t>((disk_data << 8U) | value);
        disk_shift = static_cast<std::uint16_t>((disk_shift << 8U) | value);
    ...
    void amiga500_system::tick_floppy_data_cycle() noexcept {
        ...
        drive->byte_clock_accumulator += static_cast<std::uint64_t>(drive->track_stream.size());
        while (drive->byte_clock_accumulator >= clocks_per_revolution) {
            ...
            if (write_dma) {
                transfer_disk_dma_write_byte();
            } else {
                const std::uint8_t byte = next_floppy_byte();
    ...
    void amiga500_system::transfer_disk_dma_write_byte() noexcept {
        ...
        bool should_flush_track = false;
        if (auto* drive = active_floppy_drive_state();
            drive != nullptr && !drive->track_stream.empty()) {
            if (!drive->write_protected) {
                drive->track_stream[drive->stream_offset] = byte;
                should_flush_track = disk_dma_bytes_remaining == 1U;
            }
            drive->stream_offset = (drive->stream_offset + 1U) % drive->track_stream.size();
        }
        complete_disk_dma_byte();
    ...
    void amiga500_system::start_blitter_line(std::uint32_t length) noexcept {
        ...
        std::int32_t error =
            blitter_line_error(blitter_pointer[blit_a],
                               (bltcon1 & bltcon1_line_sign) != 0U);
        ...
        blitter_pointer[blit_a] = (blitter_pointer[blit_a] & 0x001F0000U) |
    ...
    void amiga500_system::start_blitter(std::uint16_t value) noexcept {
        ...
        const bool fill_enabled =
            descending && (bltcon1 & (bltcon1_ife | bltcon1_efe)) != 0U;
    ...
    void amiga500_system::update_irq_level() noexcept {
        ...
        std::uint16_t pending = static_cast<std::uint16_t>(intena & intreq & 0x3FFFU);
        if (cia_a_irq) {
            pending = static_cast<std::uint16_t>(pending | int_ports);
        }
        if (cia_b_irq) {
            pending = static_cast<std::uint16_t>(pending | int_exter);
        }
    ...
    void amiga500_system::update_overlay_from_cia() noexcept {
        overlay_active = (cia_a.port_a_output() & 0x01U) == 0U;
    }

    std::uint32_t amiga500_system::cpu_bus_wait_cycles(std::uint32_t address, bool /*program*/,
        ...
        std::uint32_t wait = agnus.display_dma_cpu_wait_cycles();

        const std::uint16_t dma = agnus.read_dmaconr();
        const std::uint16_t nasty_mask =
            static_cast<std::uint16_t>(chips::video::agnus::dmacon_dmaen |
                                       chips::video::agnus::dmacon_blten |
                                       chips::video::agnus::dmacon_bltpri |
                                       chips::video::agnus::dmacon_bbusy);
        if ((dma & nasty_mask) == nasty_mask && blitter_cycles_remaining != 0U) {
            const std::uint32_t blitter_wait = static_cast<std::uint32_t>(
                std::min<std::uint64_t>(blitter_cycles_remaining,
                                        std::numeric_limits<std::uint32_t>::max()));
            wait = std::max(wait, blitter_wait);
        }
        return wait;
    }
        ...
        if (reader.ok()) {
            ...
            for (std::size_t drive = 0U; drive < floppy_drives.size(); ++drive) {
                ...
                if (!df.track_stream.empty()) {
                    df.stream_offset = static_cast<std::size_t>(
                        saved_floppy_stream_offset[drive] % df.track_stream.size());
                }
                if (index_lines != 0U) {
                    df.index_line_accumulator %= index_lines;
                    const std::uint64_t index_clocks =
                        static_cast<std::uint64_t>(index_lines) *
                        chips::video::agnus::color_clocks_per_line;
                    if (index_clocks != 0U) {
    ...
    std::unique_ptr<amiga500_system> assemble_amiga500(std::vector<std::uint8_t> kickstart_rom,
        ...
        s->bus.map_mmio(
            ...
            [s](std::uint32_t a, std::uint8_t v) { s->write_custom_byte(a, v); }, 0);
        s->bus.map_mmio(
        ...
        s->cpu.set_bus_wait_callback([s](std::uint32_t address, bool program, bool write) {
            return s->cpu_bus_wait_cycles(address, program, write);
        });

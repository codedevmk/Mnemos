...
#ifndef MNEMOS_IREM_M14_GAMES_DIR
#define MNEMOS_IREM_M14_GAMES_DIR ""
#endif
...
namespace {
    ...
    expected_ptrmj_regions() {
        static const std::map<std::string, expected_region, std::less<>> regions{
            {"maincpu", {.size = 0x10000U, .file_count = 8U}},
            {"gfx1", {.size = 0x0800U, .file_count = 2U}},
        };
        return regions;
    }
    ...
    [[nodiscard]] std::set<std::string, std::less<>> embedded_set_names() {
        ...
        for (const auto& [set_name, _] : mnemos::manifests::irem_m14::embedded::game_manifests) {
    ...
    [[nodiscard]] rom_set_decl require_embedded_decl(std::string_view set_name) {
        const std::string_view text = mnemos::manifests::irem_m14::game_manifest_toml(set_name);
...
TEST_CASE("m14 embedded manifests cover local ROM contracts", "[m14][romset]") {
    const auto expected_sets = embedded_set_names();
    CHECK(expected_sets == std::set<std::string, std::less<>>{"ptrmj"});

    for (const auto& [set_name, toml] : mnemos::manifests::irem_m14::embedded::game_manifests) {
        INFO("set=" << set_name);
        auto parsed =
            mnemos::manifests::common::parse_rom_set_decl(toml, "embedded:irem_m14/" +
                                                                    std::string{set_name});
        for (const auto& error : parsed.errors) {
            INFO(error.source << ":" << error.line << ":" << error.column << ": " << error.message);
        }
        ...
        CHECK(decl.board == "irem_m14");
        CHECK(decl.orientation == mnemos::manifests::common::screen_orientation::horizontal);
        CHECK_FALSE(decl.parent.has_value());
        REQUIRE(decl.regions.size() == expected_ptrmj_regions().size());

        for (const auto& [region_name, expected] : expected_ptrmj_regions()) {
...
TEST_CASE("m14 embedded manifests stay in sync with disk TOML", "[m14][romset]") {
    const std::filesystem::path games_dir{MNEMOS_IREM_M14_GAMES_DIR};
    ...
    for (const std::filesystem::directory_entry& entry :
        ...
        CHECK(normalized_text(std::string{
                  mnemos::manifests::irem_m14::game_manifest_toml(set_name)}) ==
        ...
        CHECK(parsed.value->board == "irem_m14");
...
TEST_CASE("m14 local wrapper artifacts load CRC-clean through embedded manifests",
          "[m14][romset][data]") {
    const auto dir_env = environment_value("MNEMOS_M14_SET_DIR");
    if (!dir_env.has_value() || dir_env->empty()) {
        SKIP("set MNEMOS_M14_SET_DIR to directories containing the M14 wrapper corpus");
    }

...
namespace mnemos::manifests::msx2 {
    ...
    void msx2_system::write_fdc_mmio(std::uint16_t address, std::uint8_t value) noexcept {
        ...
        case 0x04U:
            ...
            break;
        case 0x05U:
            // Canonical wd1793 sets the drive via the memory register; it does not
            // model a motor line (the idealised FDC needs no spin-up), so the
            // motor write is a no-op here.
    ...
    std::uint8_t msx2_system::io_read(std::uint16_t port) noexcept {
        ...
        case 0xD4:
            ...
            return static_cast<std::uint8_t>(((fdc.intrq() || !fdc.busy()) ? 0x80U : 0U) |
                                             (fdc.drq() ? 0x40U : 0U));
        case 0x98:
    ...
    void msx2_system::io_write(std::uint16_t port, std::uint8_t value) noexcept {
        ...
        case 0xD4: {
            ...
            fdc.write_memory_register(0x05U, drive);
            fdc.write_memory_register(
                0x04U, static_cast<std::uint8_t>(((value >> 4U) | (value >> 2U)) & 0x01U));
            break;
        }
        case 0x98:

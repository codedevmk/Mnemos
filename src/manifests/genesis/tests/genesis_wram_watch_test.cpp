...
TEST_CASE("genesis WRAM write watch (diagnostic)", "[genesis][diag]") {
    ...
    // Optional diagnostic traces, both driven from the single CPU trace
    // callback. They are composed into ONE registered callback on purpose: the
    // setter holds a single std::function, so registering them separately would
    // make the second silently replace the first.
    //
    // MNEMOS_SR_TRACE_LO/HI (hex PCs): logs each instruction whose PC is in
    // [LO,HI] as "[sr] pc=$X sr=$Y ipm=N irq=L" so the interrupt-mask +
    // pending-level can be compared at a divergence point.
    //
    // MNEMOS_REFRESH_TRACE=1: logs every instruction as "[ref] pc=$XXXXXX r=N
    // cyc=M" where r is whether a DRAM bus-refresh fired on it. cycle_sources is
    // populated at the END of an instruction, so the callback (fired at the
    // START of the NEXT instruction) reports the PREVIOUS instruction's flag --
    // we buffer one PC to align them.
    const bool sr_trace = env("MNEMOS_SR_TRACE_LO").has_value();
    const std::uint32_t sr_lo = sr_trace ? static_cast<std::uint32_t>(std::strtoul(
                                               env("MNEMOS_SR_TRACE_LO")->c_str(), nullptr, 16))
                                         : 0;
    const std::uint32_t sr_hi = env("MNEMOS_SR_TRACE_HI")
                                    ? static_cast<std::uint32_t>(std::strtoul(
                                          env("MNEMOS_SR_TRACE_HI")->c_str(), nullptr, 16))
                                    : sr_lo + 0x40U;
    ...
    if (sr_trace || refresh_trace) {
        cpu->diagnostics().set_trace_callback(
            [cpu, sr_trace, sr_lo, sr_hi, refresh_trace, &rs](std::uint32_t pc) {
                if (sr_trace) {
                    ++sr_cb_count;

...
#ifndef MNEMOS_IREM_M47_GAMES_DIR
#define MNEMOS_IREM_M47_GAMES_DIR ""
#endif
...
namespace {
    ...
    expected_effective_regions() {
        static const std::map<std::string, expected_region, std::less<>> regions{
            {"audiocpu", {.size = 0x2000U, .file_count = 2U}},
            {"gfx16x16", {.size = 0x4000U, .file_count = 4U}},
            {"gfx8x8", {.size = 0x2000U, .file_count = 2U}},
            {"maincpu", {.size = 0x8000U, .file_count = 8U}},
            {"proms", {.size = 0x0220U, .file_count = 3U}},
            {"samples", {.size = 0x2000U, .file_count = 2U}},
        };
        return regions;
    }
    ...
    [[nodiscard]] std::set<std::string, std::less<>> embedded_set_names() {
        ...
        for (const auto& [set_name, _] : mnemos::manifests::irem_m47::embedded::game_manifests) {
    ...
    [[nodiscard]] rom_set_decl require_embedded_decl(std::string_view set_name) {
        const std::string_view text = mnemos::manifests::irem_m47::game_manifest_toml(set_name);
...
TEST_CASE("m47 embedded manifests cover local ROM contracts", "[m47][romset]") {
    const auto expected_sets = embedded_set_names();
    CHECK(expected_sets == std::set<std::string, std::less<>>{"olibochu", "punchkid"});

    std::map<std::string, rom_set_decl, std::less<>> declarations;
    for (const auto& [set_name, toml] : mnemos::manifests::irem_m47::embedded::game_manifests) {
        INFO("set=" << set_name);
        auto parsed =
            mnemos::manifests::common::parse_rom_set_decl(toml, "embedded:irem_m47/" +
                                                                    std::string{set_name});
        for (const auto& error : parsed.errors) {
            INFO(error.source << ":" << error.line << ":" << error.column << ": " << error.message);
        }
...
TEST_CASE("m47 embedded manifests stay in sync with disk TOML", "[m47][romset]") {
    const std::filesystem::path games_dir{MNEMOS_IREM_M47_GAMES_DIR};
    ...
    for (const std::filesystem::directory_entry& entry :
        ...
        CHECK(normalized_text(std::string{
                  mnemos::manifests::irem_m47::game_manifest_toml(set_name)}) ==
        ...
        CHECK(parsed.value->board == "irem_m47");
...
TEST_CASE("m47 local wrapper artifacts load CRC-clean through embedded manifests",
          "[m47][romset][data]") {
    const auto dir_env = environment_value("MNEMOS_M47_SET_DIR");
    if (!dir_env.has_value() || dir_env->empty()) {
        SKIP("set MNEMOS_M47_SET_DIR to directories containing the M47 wrapper corpus");
    }

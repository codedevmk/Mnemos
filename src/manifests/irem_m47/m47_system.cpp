...
namespace mnemos::manifests::irem_m47 {
    ...
    namespace {
        ...
        [[nodiscard]] std::uint8_t tile_pixel_pen(std::span<const std::uint8_t> tile_gfx,
            ...
            std::uint8_t pen = 0U;
            const std::uint16_t tile = static_cast<std::uint16_t>(code & 0x01FFU);
            for (std::uint32_t plane = 0; plane < 2U; ++plane) {
                const std::uint64_t base =
                    static_cast<std::uint64_t>(plane) * 0x1000U +
        ...
        [[nodiscard]] std::uint32_t board_identity_crc(const common::rom_set_image& roms,
                                                       const m47_board_params& params) {
            std::uint32_t crc =
                security::cryptography::crc32(std::string_view{"m47.board.identity.v1"});
    ...
    m47_board_params board_params_for(std::string_view set_name) noexcept {
        if (set_name == "olibochu" || set_name == "punchkid") {
            return {.rom_layout = "m47",
                    .dsw1_default = m47_dsw1_default,
                    .dsw2_default = m47_dsw2_default};
        }
        return {};
    }

    m47_video::m47_video() : pixels_(static_cast<std::size_t>(visible_width) * visible_height, 0U) {
        reset(chips::reset_kind::power_on);
    }

    chips::chip_metadata m47_video::metadata() const noexcept {
        return {.manufacturer = "Irem",
                .part_number = "m47_video_first_pass",
                .family = "irem_m47",
                .klass = chips::chip_class::video,
                .revision = 1U};
    }

    void m47_video::tick(std::uint64_t cycles) { elapsed_cycles_ += cycles; }

    void m47_video::reset(chips::reset_kind /*kind*/) {
        std::fill(pixels_.begin(), pixels_.end(), 0U);
        elapsed_cycles_ = 0U;
        frame_index_ = 0U;
    }

    chips::frame_buffer_view m47_video::framebuffer() const noexcept {
    ...
    void m47_video::compose(std::span<const std::uint8_t> tile_gfx,
    ...
    void m47_video::save_state(chips::state_writer& writer) const {
    ...
    void m47_video::load_state(chips::state_reader& reader) {
    ...
    m47_system::m47_system(common::rom_set_image image, m47_board_params board_params)
        : roms(std::move(image)), params(board_params) {
        auto& main_prog = pinned_region(roms, "maincpu", main_rom_size);
        auto& sound_prog = pinned_region(roms, "audiocpu", sound_rom_size);
        (void)pinned_region(roms, "samples", samples_size);
        (void)pinned_region(roms, "gfx8x8", tile_gfx_size);
        (void)pinned_region(roms, "gfx16x16", sprite_gfx_size);
    ...
    void m47_system::run_frame() {
        ...
        const auto* tile_gfx = roms.region("gfx8x8");
        const auto* sprite_gfx = roms.region("gfx16x16");
    ...
    void m47_system::set_inputs(std::uint8_t p1, std::uint8_t p2, std::uint8_t system) noexcept {
        input0 = p1;
        input1 = p2;
        input2 = system;
    }

    void m47_system::latch_sound_command(std::uint8_t value) noexcept {
        sound_command = value;
    ...
    void m47_system::update_sound_irq() noexcept { sound_cpu.set_irq_line(sound_latch_irq); }

    void m47_system::save_state(chips::state_writer& writer) const {
        writer.u32(m47_system_state_version);
    ...
    void m47_system::load_state(chips::state_reader& reader) {
        if (reader.u32() != m47_system_state_version) {
            reader.fail();
            return;
        }
    ...
    std::unique_ptr<m47_system> assemble_m47(common::rom_set_image image,
                                             m47_board_params board_params) {
        return std::make_unique<m47_system>(std::move(image), board_params);
    }

...
#ifndef MNEMOS_IREM_M57_GAMES_DIR
#define MNEMOS_IREM_M57_GAMES_DIR ""
#endif
...
namespace {
    ...
    [[nodiscard]] std::set<std::string, std::less<>> embedded_set_names() {
        ...
        for (const auto& [set_name, _] : mnemos::manifests::irem_m57::embedded::game_manifests) {
    ...
    [[nodiscard]] rom_set_decl require_embedded_decl(std::string_view set_name) {
        const std::string_view text = mnemos::manifests::irem_m57::game_manifest_toml(set_name);
...
TEST_CASE("m57 embedded manifests cover local raw-media contracts", "[m57][romset]") {
    const auto& expected = expected_contracts();
    std::set<std::string, std::less<>> names;

    for (const auto& [set_name, toml] : mnemos::manifests::irem_m57::embedded::game_manifests) {
        INFO("set=" << set_name);
        const auto contract = expected.find(set_name);
        REQUIRE(contract != expected.end());

        auto parsed =
            mnemos::manifests::common::parse_rom_set_decl(toml, "embedded:irem_m57/" +
                                                                    std::string{set_name});
        for (const auto& error : parsed.errors) {
            INFO(error.source << ":" << error.line << ":" << error.column << ": " << error.message);
        }
...
TEST_CASE("m57 embedded manifests stay in sync with disk TOML", "[m57][romset]") {
    const std::filesystem::path games_dir{MNEMOS_IREM_M57_GAMES_DIR};
    ...
    for (const std::filesystem::directory_entry& entry :
        ...
        CHECK(normalized_text(std::string{
                  mnemos::manifests::irem_m57::game_manifest_toml(set_name)}) ==
        ...
        CHECK(parsed.value->board == "irem_m57");
...
TEST_CASE("m57 local wrapper artifacts load CRC-clean through embedded manifests",
          "[m57][romset][data]") {
    const auto dir_env = environment_value("MNEMOS_M57_SET_DIR");
    if (!dir_env.has_value() || dir_env->empty()) {
        SKIP("set MNEMOS_M57_SET_DIR to directories containing the M57 wrapper corpus");
    }

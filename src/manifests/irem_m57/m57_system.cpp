...
namespace mnemos::manifests::irem_m57 {
    ...
    namespace {
        ...
        [[nodiscard]] std::uint8_t layout_code(std::string_view layout) noexcept {
            if (layout == "m57_newtangl_z80_raw_media") {
                return 1U;
            }
            return 0U;
        }
        ...
        [[nodiscard]] std::uint32_t board_identity_crc(const common::rom_set_image& roms,
                                                       const m57_board_params& params) {
            std::uint32_t crc =
                security::cryptography::crc32(std::string_view{"m57.board.identity.v1"});
        ...
        [[nodiscard]] std::uint8_t read_main_rom_byte(const m57_system& system,
    ...
    m57_board_params board_params_for(std::string_view set_name) noexcept {
        if (!set_name.empty()) {
            return {.cpu_clock_hz = cpu_clock_hz,
                    .rom_layout = "m57_newtangl_z80_raw_media",
                    .dip_default = newtangl_dip_default};
        }
        return {};
    }

    std::uint8_t m57_cpu_bus::read8(std::uint32_t address) {
        if (system_ == nullptr) {
            return 0xFFU;
        }
    ...
    void m57_cpu_bus::write8(std::uint32_t address, std::uint8_t value) {
        if (system_ == nullptr) {
            return;
        }
    ...
    chips::chip_metadata m57_video::metadata() const noexcept {
        return {.manufacturer = "Irem",
                .part_number = "m57_video_first_pass",
                .family = "irem_m57",
                .klass = chips::chip_class::video,
                .revision = 1U};
    }

    void m57_video::tick(std::uint64_t cycles) { elapsed_cycles_ += cycles; }

    void m57_video::reset(chips::reset_kind /*kind*/) {
        std::fill(pixels_.begin(), pixels_.end(), 0U);
        elapsed_cycles_ = 0U;
        frame_index_ = 0U;
    }

    chips::frame_buffer_view m57_video::framebuffer() const noexcept {
    ...
    void m57_video::compose(std::span<const std::uint8_t> video_ram,
    ...
    void m57_video::save_state(chips::state_writer& writer) const {
    ...
    void m57_video::load_state(chips::state_reader& reader) {
    ...
    m57_system::m57_system(common::rom_set_image image, m57_board_params board_params)
    ...
    void m57_system::set_inputs(std::uint8_t p1, std::uint8_t p2,
    ...
    std::uint8_t m57_system::read_io_port(std::uint16_t port) const noexcept {
        switch (port & 0x00FFU) {
        case 0x00U:
            return input_p1;
        case 0x01U:
    ...
    void m57_system::write_io_port(std::uint16_t port, std::uint8_t value) noexcept {
        switch (port & 0x00FFU) {
        case 0x04U:
    ...
    void m57_system::write_sound_latch(std::uint8_t value) noexcept {
    ...
    void m57_system::save_state(chips::state_writer& writer) const {
        writer.u32(m57_system_state_version);
    ...
    void m57_system::load_state(chips::state_reader& reader) {
        if (reader.u32() != m57_system_state_version) {
            reader.fail();
            return;
        }
    ...
    std::unique_ptr<m57_system> assemble_m57(common::rom_set_image image,
                                             m57_board_params board_params) {
        return std::make_unique<m57_system>(std::move(image), board_params);
    }

...
namespace mnemos::manifests::odyssey {
    ...
    void odyssey_system::set_ball_speed(float speed) noexcept {
        const auto magnitude = std::clamp(rounded((clamp_unit(speed) + 1.0F) * 2.0F),
                                          std::int16_t{1}, std::int16_t{4});
        ball_dx_ = static_cast<std::int16_t>((ball_dx_ < 0) ? -magnitude : magnitude);
    }

    void odyssey_system::reset() noexcept {
        player0_ = {.x = map_x(-0.82F),
                    .y = map_y(0.0F),
                    .w = 6U,
                    .h = 6U,
                    .visible = wiring_.player0_visible};
        player1_ = {.x = map_x(0.82F),
                    .y = map_y(0.0F),
                    .w = 6U,
                    .h = 6U,
    ...
    void odyssey_system::draw_rect(odyssey_sprite sprite) noexcept {
        ...
        for (std::uint8_t y = 0U; y < sprite.h; ++y) {
            ...
            for (std::uint8_t x = 0U; x < sprite.w; ++x) {
                ...
                frame_.luma[static_cast<std::size_t>(py) * frame_.width +
                            static_cast<std::size_t>(px)] = white_luma;
            }
        }
    }
    ...
    void odyssey_system::draw_center_line() noexcept {
        ...
        for (std::int16_t y = static_cast<std::int16_t>(center_y - half_h);
             y < static_cast<std::int16_t>(center_y + half_h); ++y) {
            ...
            for (std::int16_t x = line_x_; x < static_cast<std::int16_t>(line_x_ + 2); ++x) {
                ...
                frame_.luma[static_cast<std::size_t>(y) * frame_.width +
                            static_cast<std::size_t>(x)] = white_luma;
            }
        }
    }

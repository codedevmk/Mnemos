...
namespace mnemos::manifests::irem_m81 {
    ...
    namespace {
        ...
        [[nodiscard]] std::uint8_t layout_code(std::string_view layout) noexcept {
            if (layout == "single_boot_pair") {
                return 1U;
            }
            if (layout == "small_boot_pair") {
                return 2U;
            }
            if (layout == "large_boot_pair") {
                return 3U;
            }
            return 0U;
        }
        ...
        [[nodiscard]] std::uint32_t rom_identity_crc(const common::rom_set_image& roms,
                                                     const m81_board_params& params) {
            std::uint32_t crc =
                security::cryptography::crc32(std::string_view{"m81.rom.identity.v1"});
    ...
    m81_board_params board_params_for(std::string_view set_name) noexcept {
        if (set_name == "dbreed") {
            return {.dip_default = 0xF5FFU, .rom_layout = "single_boot_pair"};
        }
        if (set_name == "hharry") {
            return {.dip_default = 0xFDBFU, .rom_layout = "small_boot_pair"};
        }
        if (set_name == "xmultipl") {
            return {.dip_default = 0xFDFFU, .rom_layout = "large_boot_pair"};
        }
        return {};
    }

    m81_video::m81_video() : pixels_(static_cast<std::size_t>(visible_width) * visible_height, 0U) {
        reset(chips::reset_kind::power_on);
    }

    chips::chip_metadata m81_video::metadata() const noexcept {
        return {.manufacturer = "Irem",
                .part_number = "m81_video",
                .family = "irem_m81",
                .klass = chips::chip_class::video,
                .revision = 1U};
    }

    void m81_video::tick(std::uint64_t cycles) { elapsed_cycles_ += cycles; }

    void m81_video::reset(chips::reset_kind /*kind*/) {
    ...
    chips::frame_buffer_view m81_video::framebuffer() const noexcept {
    ...
    void m81_video::latch_sprites(std::span<const std::uint8_t> sprite_ram) noexcept {
        sprite_buffer_.fill(0U);
        const std::size_t count = std::min(sprite_ram.size(), sprite_buffer_.size());
        std::copy_n(sprite_ram.begin(), count, sprite_buffer_.begin());
    }

    void m81_video::begin_frame() noexcept { std::fill(pixels_.begin(), pixels_.end(), 0U); }

    void m81_video::compose_scanline(std::span<const std::uint8_t> tiles,
        ...
        for (std::uint32_t x = 0; x < visible_width; ++x) {
            ...
            const std::uint8_t prom =
                sample_byte(proms, (tile ^ sprite) + (frame_index_ & 0xFFU), tint);
    ...
    void m81_video::end_frame() noexcept { ++frame_index_; }

    void m81_video::compose(std::span<const std::uint8_t> tiles,
                            std::span<const std::uint8_t> sprites,
                            std::span<const std::uint8_t> proms, std::span<const std::uint8_t> vram,
                            std::span<const std::uint8_t> rowscroll,
                            std::span<const std::uint8_t> palette, std::string_view rom_layout) {
        begin_frame();
        for (std::uint32_t line = 0; line < visible_height; ++line) {
            compose_scanline(tiles, sprites, proms, vram, rowscroll, palette, line, rom_layout);
        }
        end_frame();
    }

    void m81_video::save_state(chips::state_writer& writer) const {
    ...
    void m81_video::load_state(chips::state_reader& reader) {
    ...
    m81_system::m81_system(common::rom_set_image image, m81_board_params board_params)
    ...
    void m81_system::set_inputs(std::uint8_t p1, std::uint8_t p2, std::uint8_t system) noexcept {
        input_p1 = p1;
        input_p2 = p2;
        input_system = system;
    }

    void m81_system::update_sound_irq() noexcept {
    ...
    void m81_system::discard_dac_write_events_before(std::uint64_t sound_clock) {
    ...
    void m81_system::save_state(chips::state_writer& writer) const {
        writer.u32(m81_system_state_version);
    ...
    void m81_system::load_state(chips::state_reader& reader) {
        if (reader.u32() != m81_system_state_version) {
            reader.fail();
            return;
        }
    ...
    std::unique_ptr<m81_system> assemble_m81(common::rom_set_image image,
                                             m81_board_params board_params) {
        return std::make_unique<m81_system>(std::move(image), board_params);
    }

...
namespace mnemos::manifests::irem_m81 {
    ...
    inline constexpr std::size_t proms_size = 0x00065CU;
    inline constexpr std::uint32_t m81_system_state_version = 2U;
    ...
    struct m81_board_params final {
        std::uint16_t dip_default{0xFFFFU};
        std::string_view rom_layout{"standard"};
    };

    [[nodiscard]] m81_board_params board_params_for(std::string_view set_name) noexcept;

    class m81_video final : public chips::ivideo {
      public:
        m81_video();
    ...
    struct m81_system final {
        ...
        explicit m81_system(common::rom_set_image image, m81_board_params board_params = {});
    ...
    [[nodiscard]] std::unique_ptr<m81_system> assemble_m81(common::rom_set_image image,

...
namespace {
    ...
    [[nodiscard]] std::vector<std::uint8_t> glyph_ramp() {
        std::vector<std::uint8_t> gfx(mnemos::manifests::irem_m63::gfx_rom_size);
...
TEST_CASE("Irem M63 system runs the first-pass Z80 diagnostic memory map", "[irem_m63]") {
    namespace M63 = mnemos::manifests::irem_m63;

    auto sys = M63::assemble_m63(synthetic_image(), M63::board_params_for("wilytowr"));
    CHECK(sys->dip_switches == M63::wilytowr_dip_default);
    CHECK(sys->video.framebuffer().width == M63::visible_width);
    CHECK(sys->video.framebuffer().height == M63::visible_height);
    REQUIRE(sys->roms.region("gfx_combined") != nullptr);
    CHECK(sys->roms.region("gfx_combined")->size() == M63::gfx_rom_size);
...
TEST_CASE("Irem M63 video output is GFX-ROM and RAM driven", "[irem_m63]") {
    namespace M63 = mnemos::manifests::irem_m63;

    std::array<std::uint8_t, M63::video_ram_size> video_ram{};
    std::array<std::uint8_t, M63::color_ram_size> color_ram{};
    ...
    M63::m63_video first;
    first.compose(video_ram, color_ram, gfx, program, false,
                  "m63_wilytower_z80_artifact_media");
    M63::m63_video second;
    second.compose(video_ram, color_ram, gfx, program, false,
                   "m63_wilytower_z80_artifact_media");

    CHECK(frame_pixels(first.framebuffer()) == frame_pixels(second.framebuffer()));
    CHECK(nonblack_pixels(first.framebuffer()) > 0U);

    M63::m63_video flipped;
    flipped.compose(video_ram, color_ram, gfx, program, true,
                    "m63_wilytower_z80_artifact_media");
    CHECK(frame_pixels(flipped.framebuffer()) != frame_pixels(first.framebuffer()));
}

TEST_CASE("Irem M63 save-state preserves board identity and RAM", "[irem_m63]") {
    namespace M63 = mnemos::manifests::irem_m63;

    auto source = M63::assemble_m63(synthetic_image(), M63::board_params_for("wilytowr"));
    source->run_frame();
    ...
    auto restored = M63::assemble_m63(synthetic_image(), M63::board_params_for("wilytowr"));
    ...
    auto wrong = M63::assemble_m63(
        synthetic_image(),
        {.cpu_clock_hz = M63::cpu_clock_hz, .rom_layout = "unknown", .dip_default = 0x00U});

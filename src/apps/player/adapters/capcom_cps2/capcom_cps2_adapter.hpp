...
namespace mnemos::apps::player::adapters::capcom_cps2 {
    ...
    //
    // Input mapping (first-cut, active low): the joystick word packs P1 in the low
    // byte and P2 in the high byte (up/down/left/right in bits 0-3, buttons 1-3 in
    // bits 4-6); the system word carries coin 1/2 (the pads' `select`) and start
    // 1/2. Audio is the QSound DL-1425 stereo stream, drained per frame.
    class capcom_cps2_adapter final : public frontend_sdk::player_system {
      public:
        ...
        [[nodiscard]] frontend_sdk::video_region region() const noexcept override {
            // ~59.6 Hz progressive raster, 384x224. Orientation is horizontal for
            // now; vertical (TATE) sets are a later refinement.

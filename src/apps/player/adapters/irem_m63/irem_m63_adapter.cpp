...
namespace mnemos::apps::player::adapters::irem_m63 {
    ...
    namespace {
        ...
        frontend_sdk::session_capability_info make_session_capabilities() {
            ...
            session.input_ports = {
                {.port_index = 0U,
                 ...
                 .device_id = "irem_m63.panel.p1",
                 .label = "Player 1 Panel"},
                {.port_index = 1U,
                 .player_slot = 2U,
                 .format = frontend_sdk::input_device_format::arcade_panel,
                 .device_id = "irem_m63.panel.p2",
        ...
        [[nodiscard]] std::string resident_media_crc32(const loaded_set& set) {
            ...
            std::uint32_t crc = mnemos::security::cryptography::crc32("irem_m63.resident_media.v1");
        ...
        parse_irem_decl(std::string_view text, std::string source,
            ...
            if (!parsed.ok()) {
                for (const auto& error : parsed.errors) {
                    std::fprintf(stderr, "[irem_m63] %s:%u:%u: %s\n", error.source.c_str(),
            ...
            if (parsed.value->board != "irem_m63") {
                std::fprintf(stderr, "[irem_m63] %s declares board '%s', expected 'irem_m63'\n",
        ...
        embedded_decl_for_set(std::string_view set_name) {
            const std::string_view toml = M63::game_manifest_toml(set_name);
            if (toml.empty()) {
                return std::nullopt;
            }
            return parse_irem_decl(toml, "embedded:irem_m63/" + std::string(set_name) + ".toml",
                                   set_name);
        }
        ...
        load_declared_set(mnemos::manifests::common::rom_set_decl decl,
            ...
            for (const auto& issue : result.image.issues) {
                std::fprintf(stderr, "[irem_m63] %s: %s\n", issue.file.c_str(),
                             issue.message.c_str());
            }
            return result;
        }
        ...
        [[nodiscard]] std::unique_ptr<M63::m63_system> assemble_from(loaded_set set) {
            return M63::assemble_m63(std::move(set.image), M63::board_params_for(set.set_name));
        }

        constexpr std::uint32_t irem_m63_adapter_state_version = 1U;
        constexpr std::uint32_t irem_m63_adapter_save_target_manifest_rev = 1U;
    ...
    irem_m63_adapter::irem_m63_adapter(std::vector<std::uint8_t> rom, std::string display_name,
        ...
        spec_ = {{"System", "Arcade"}, {"Board", "Irem M63"}, {"Game", game_label}};
    }

    void irem_m63_adapter::publish_memory_views() {
    ...
    void irem_m63_adapter::sync_inputs_from_ports() noexcept {
        const auto pack_controls = [](const frontend_sdk::controller_state& c) -> std::uint8_t {
            std::uint8_t value = 0U;
            if (c.up) {
                value |= M63::panel_up_bit;
            }
            if (c.down) {
                value |= M63::panel_down_bit;
            }
            if (c.left) {
                value |= M63::panel_left_bit;
            }
            if (c.right) {
                value |= M63::panel_right_bit;
            }
            if (c.a) {
                value |= M63::panel_button1_bit;
            }
            if (c.b) {
                value |= M63::panel_button2_bit;
            }
            return value;
        };
        std::uint8_t system = 0U;
        if (ports_[0].start) {
            system |= M63::start1_bit;
        }
        if (ports_[1].start) {
            system |= M63::start2_bit;
        }
        if (ports_[0].select || ports_[1].select) {
            system |= M63::coin1_bit;
        }
        if (ports_[0].service || ports_[0].mode) {
            system |= M63::service_bit;
        }
        sys_->set_inputs(pack_controls(ports_[0]), pack_controls(ports_[1]), system);
    }

    void irem_m63_adapter::save_adapter_state(chips::state_writer& writer) const {
        writer.u32(irem_m63_adapter_state_version);
    ...
    void irem_m63_adapter::load_adapter_state(chips::state_reader& reader) {
        if (reader.u32() != irem_m63_adapter_state_version) {
            reader.fail();
            return;
        }
    ...
    void irem_m63_adapter::step_one_frame() {
        sys_->run_frame();
        ++frames_stepped_;
    }

    void irem_m63_adapter::apply_input(int port,
    ...
    frontend_sdk::audio_chunk irem_m63_adapter::drain_audio() noexcept {
        ...
        return {.samples = audio_buf_.data(),
                ...
                .sample_rate = M63::audio_rate_hz};
    }

    std::vector<std::uint8_t> irem_m63_adapter::save_state() {
        return runtime::write_save_state(build_save_target(*this));
    }

    runtime::load_result irem_m63_adapter::load_state(std::span<const std::uint8_t> data) {
    ...
    runtime::save_target build_save_target(manifests::irem_m63::m63_system& sys) {
        runtime::save_target target;
        target.manifest_id = "irem_m63";
        target.manifest_rev = manifests::irem_m63::m63_system_state_version;
    ...
    runtime::save_target build_save_target(irem_m63_adapter& adapter) {
        runtime::save_target target;
        target.manifest_id = "irem_m63.adapter";
        target.manifest_rev = irem_m63_adapter_save_target_manifest_rev;
        target.components.push_back(
            {"board",
        ...
        const auto register_irem_m63 = [] {
            mnemos::frontend_sdk::adapter_registry::instance().register_family(
                ...
                [](mnemos::frontend_sdk::adapter_options opts)
                    ...
                    return std::make_unique<irem_m63_adapter>(

...
namespace {
    ...
    namespace irem = mnemos::apps::player::adapters::irem_m63;
    namespace M63 = mnemos::manifests::irem_m63;

    [[nodiscard]] std::vector<std::uint8_t> make_m63_program() {
        ...
        const std::vector<std::uint8_t> program{
            0x3EU, 0x42U, 0x32U, lo(M63::scratch_ram_base), hi(M63::scratch_ram_base),
            0x3EU, 0x81U, 0x32U, lo(M63::video_ram_base), hi(M63::video_ram_base),
            0x3EU, 0x07U, 0x32U, lo(M63::color_ram_base), hi(M63::color_ram_base),
            0x3EU, 0x01U, 0x32U, lo(M63::sound_latch_address), hi(M63::sound_latch_address),
            0x3EU, 0x01U, 0x32U, lo(M63::control_register_address),
            hi(M63::control_register_address),
            0xC3U, 0x1EU, 0x00U}; // JP $001E
        std::copy(program.begin(), program.end(), rom.begin() + M63::program_rom_base);
        return rom;
    }
    ...
    [[nodiscard]] std::set<std::string, std::less<>> embedded_set_names() {
        ...
        for (const auto& [set_name, _] : M63::embedded::game_manifests) {
...
TEST_CASE("irem_m63_adapter boots a synthetic M63 program", "[irem_m63]") {
    irem::irem_m63_adapter adapter(make_m63_program(), "Tiny M63");
    CHECK(adapter.region().frames_per_second_x1000 == M63::frame_rate_x1000);
    CHECK(adapter.region().orientation == mnemos::frontend_sdk::display_orientation::horizontal);
    CHECK(adapter.current_frame().width == M63::visible_width);
    CHECK(adapter.current_frame().height == M63::visible_height);
...
TEST_CASE("irem_m63_adapter save state restores board and adapter state", "[irem_m63]") {
    irem::irem_m63_adapter source(make_m63_program(), "Tiny M63");
    ...
    CHECK(source.machine().input_p1 ==
          static_cast<std::uint8_t>(M63::panel_button1_bit | M63::panel_left_bit));
    CHECK(source.machine().input_system == M63::start1_bit);

    p1.select = true;
    source.apply_input(0, p1);
    CHECK(source.machine().input_system ==
          static_cast<std::uint8_t>(M63::start1_bit | M63::coin1_bit));

    const auto state = source.save_state();
    REQUIRE_FALSE(state.empty());

    irem::irem_m63_adapter restored(make_m63_program(), "Tiny M63");
...
TEST_CASE("irem_m63_adapter drains first-pass beeper audio", "[irem_m63]") {
    irem::irem_m63_adapter adapter(make_m63_program(), "Audio M63");
    adapter.step_one_frame();

    const auto audio = adapter.drain_audio();
    CHECK(audio.frame_count > 0U);
    CHECK(audio.sample_rate == M63::audio_rate_hz);
    CHECK(audio.samples != nullptr);
}

TEST_CASE("irem_m63_adapter validates real M63 ROM sets", "[irem_m63][data]") {
    const auto dir_env = environment_value("MNEMOS_M63_SET_DIR");
    if (!dir_env.has_value() || dir_env->empty()) {
        SKIP("set MNEMOS_M63_SET_DIR to directories containing the M63 zip/folder corpus");
    }
    ...
    if (!missing_sets.empty()) {
        ...
        FAIL("missing M63 artifacts: " << missing.str());
    }
    ...
    for (const auto& set_name : expected_sets) {
        ...
        irem::irem_m63_adapter adapter(read_source_bytes(source_it->second), set_name, nullptr, {},

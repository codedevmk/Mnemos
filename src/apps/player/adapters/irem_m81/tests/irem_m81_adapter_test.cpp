...
namespace {
    ...
    namespace irem = mnemos::apps::player::adapters::irem_m81;
    namespace m81 = mnemos::manifests::irem_m81;

    [[nodiscard]] std::vector<std::uint8_t> synthetic_m81_program() {
        std::vector<std::uint8_t> rom(m81::main_rom_size, 0xFFU);
    ...
    [[nodiscard]] std::set<std::string, std::less<>> embedded_set_names() {
        ...
        for (const auto& [set_name, _] : m81::embedded::game_manifests) {
    ...
    [[nodiscard]] std::optional<std::string>
    identify_m81_source(const std::filesystem::path& path,
    ...
    index_m81_source_roots(const std::vector<std::filesystem::path>& roots) {
        ...
        auto maybe_add = [&](const std::filesystem::path& path) {
            auto set_id = identify_m81_source(path, known);
...
TEST_CASE("irem_m81_adapter boots a synthetic M81 program", "[irem_m81]") {
    irem::irem_m81_adapter adapter(synthetic_m81_program(), "Tiny M81");

    CHECK(adapter.region().frames_per_second_x1000 == m81::frame_rate_x1000);
    CHECK(adapter.current_frame().width == m81::visible_width);
    CHECK(adapter.current_frame().height == m81::visible_height);
...
TEST_CASE("irem_m81_adapter save state restores board and adapter state", "[irem_m81]") {
    irem::irem_m81_adapter source(synthetic_m81_program(), "Tiny M81");
    ...
    irem::irem_m81_adapter restored(synthetic_m81_program(), "Tiny M81");
...
TEST_CASE("irem_m81_adapter validates real M81 ROM sets", "[irem_m81][data]") {
    ...
    const auto sources = index_m81_source_roots(roots);
    ...
    for (const auto& set_name : expected_sets) {
        ...
        irem::irem_m81_adapter adapter(read_source_bytes(source_path), set_name, nullptr, {},
                                       source_path.string());

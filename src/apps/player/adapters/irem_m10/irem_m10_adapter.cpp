...
namespace mnemos::apps::player::adapters::irem_m10 {
    ...
    namespace {
        ...
        frontend_sdk::session_capability_info make_session_capabilities() {
            ...
            session.input_ports = {
                {.port_index = 0U,
                 ...
                 .device_id = "irem_m10.panel.p1",
                 .label = "Player 1 Panel"},
                {.port_index = 1U,
                 .player_slot = 2U,
                 .format = frontend_sdk::input_device_format::arcade_panel,
                 .device_id = "irem_m10.panel.p2",
        ...
        [[nodiscard]] std::string resident_media_crc32(const loaded_set& set) {
            ...
            std::uint32_t crc = mnemos::security::cryptography::crc32("irem_m10.resident_media.v1");
        ...
        parse_irem_decl(std::string_view text, std::string source,
            ...
            if (!parsed.ok()) {
                for (const auto& error : parsed.errors) {
                    std::fprintf(stderr, "[irem_m10] %s:%u:%u: %s\n", error.source.c_str(),
            ...
            if (parsed.value->board != "irem_m10") {
                std::fprintf(stderr, "[irem_m10] %s declares board '%s', expected 'irem_m10'\n",
        ...
        embedded_decl_for_set(std::string_view set_name) {
            const std::string_view toml = M10::game_manifest_toml(set_name);
            if (toml.empty()) {
                return std::nullopt;
            }
            return parse_irem_decl(toml, "embedded:irem_m10/" + std::string(set_name) + ".toml",
                                   set_name);
        }
        ...
        load_declared_set(mnemos::manifests::common::rom_set_decl decl,
            ...
            for (const auto& issue : result.image.issues) {
                std::fprintf(stderr, "[irem_m10] %s: %s\n", issue.file.c_str(),
                             issue.message.c_str());
            }
            return result;
        }
        ...
        [[nodiscard]] std::unique_ptr<M10::m10_system> assemble_from(loaded_set set) {
            return M10::assemble_m10(std::move(set.image), M10::board_params_for(set.set_name));
        }

        constexpr std::uint32_t irem_m10_adapter_state_version = 1U;
        constexpr std::uint32_t irem_m10_adapter_save_target_manifest_rev = 1U;
    ...
    irem_m10_adapter::irem_m10_adapter(std::vector<std::uint8_t> rom, std::string display_name,
        ...
        spec_ = {{"System", "Arcade"}, {"Board", "Irem M10"}, {"Game", game_label}};
    }

    void irem_m10_adapter::publish_memory_views() {
    ...
    void irem_m10_adapter::sync_inputs_from_ports() noexcept {
        const auto pack_controls = [](const frontend_sdk::controller_state& c) -> std::uint8_t {
            std::uint8_t value = 0U;
            if (c.up) {
                value |= M10::panel_up_bit;
            }
            if (c.down) {
                value |= M10::panel_down_bit;
            }
            if (c.left) {
                value |= M10::panel_left_bit;
            }
            if (c.right) {
                value |= M10::panel_right_bit;
            }
            if (c.a) {
                value |= M10::panel_button1_bit;
            }
            if (c.b) {
                value |= M10::panel_button2_bit;
            }
            return value;
        };
        std::uint8_t system = 0U;
        if (ports_[0].start) {
            system |= M10::start1_bit;
        }
        if (ports_[1].start) {
            system |= M10::start2_bit;
        }
        if (ports_[0].select || ports_[1].select) {
            system |= M10::coin1_bit;
        }
        if (ports_[0].service || ports_[0].mode) {
            system |= M10::service_bit;
        }
        sys_->set_inputs(pack_controls(ports_[0]), pack_controls(ports_[1]), system);
    }

    void irem_m10_adapter::save_adapter_state(chips::state_writer& writer) const {
        writer.u32(irem_m10_adapter_state_version);
    ...
    void irem_m10_adapter::load_adapter_state(chips::state_reader& reader) {
        if (reader.u32() != irem_m10_adapter_state_version) {
            reader.fail();
            return;
        }
    ...
    void irem_m10_adapter::step_one_frame() {
        sys_->run_frame();
        ++frames_stepped_;
    }

    void irem_m10_adapter::apply_input(int port,
    ...
    frontend_sdk::audio_chunk irem_m10_adapter::drain_audio() noexcept {
        ...
        return {.samples = audio_buf_.data(),
                ...
                .sample_rate = M10::audio_rate_hz};
    }

    std::vector<std::uint8_t> irem_m10_adapter::save_state() {
        return runtime::write_save_state(build_save_target(*this));
    }

    runtime::load_result irem_m10_adapter::load_state(std::span<const std::uint8_t> data) {
    ...
    runtime::save_target build_save_target(manifests::irem_m10::m10_system& sys) {
        runtime::save_target target;
        target.manifest_id = "irem_m10";
        target.manifest_rev = manifests::irem_m10::m10_system_state_version;
    ...
    runtime::save_target build_save_target(irem_m10_adapter& adapter) {
        runtime::save_target target;
        target.manifest_id = "irem_m10.adapter";
        target.manifest_rev = irem_m10_adapter_save_target_manifest_rev;
        target.components.push_back(
            {"board",
        ...
        const auto register_irem_m10 = [] {
            mnemos::frontend_sdk::adapter_registry::instance().register_family(
                ...
                [](mnemos::frontend_sdk::adapter_options opts)
                    ...
                    return std::make_unique<irem_m10_adapter>(

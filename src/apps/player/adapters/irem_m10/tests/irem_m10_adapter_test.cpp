...
namespace {
    ...
    namespace irem = mnemos::apps::player::adapters::irem_m10;
    namespace M10 = mnemos::manifests::irem_m10;

    [[nodiscard]] std::vector<std::uint8_t> make_m10_program() {
        std::vector<std::uint8_t> rom(M10::main_rom_size, 0xFFU);
        rom[0x0000U] = 0xC3U; // JP $1000 bootstrap for the M10 vector window.
        ...
        const std::vector<std::uint8_t> program{
            0x3EU, 0x42U, 0x32U, 0x00U, 0x00U, // LD A,$42 ; LD ($0000),A
            0x3EU, 0x81U, 0x32U, 0x00U, 0x40U, // LD A,$81 ; LD ($4000),A
            0x3EU, 0x07U, 0x32U, 0x00U, 0x48U, // LD A,$07 ; LD ($4800),A
            0x3EU, 0x01U, 0x32U, 0x04U, 0x60U, // LD A,$01 ; LD ($6004),A
            0x3EU, 0x01U, 0x32U, 0x05U, 0x60U, // LD A,$01 ; LD ($6005),A
            0xC3U, 0x1EU, 0x10U};             // JP $101E
        std::copy(program.begin(), program.end(), rom.begin() + M10::program_rom_base);
        return rom;
    }
    ...
    [[nodiscard]] std::set<std::string, std::less<>> embedded_set_names() {
        ...
        for (const auto& [set_name, _] : M10::embedded::game_manifests) {
...
TEST_CASE("irem_m10_adapter boots a synthetic M10 program", "[irem_m10]") {
    irem::irem_m10_adapter adapter(make_m10_program(), "Tiny M10");
    CHECK(adapter.region().frames_per_second_x1000 == M10::frame_rate_x1000);
    CHECK(adapter.region().orientation == mnemos::frontend_sdk::display_orientation::vertical);
    CHECK(adapter.current_frame().width == M10::visible_width);
    CHECK(adapter.current_frame().height == M10::visible_height);
...
TEST_CASE("irem_m10_adapter save state restores board and adapter state", "[irem_m10]") {
    irem::irem_m10_adapter source(make_m10_program(), "Tiny M10");
    ...
    CHECK(source.machine().input_p1 ==
          static_cast<std::uint8_t>(M10::panel_button1_bit | M10::panel_left_bit));
    CHECK(source.machine().input_system == M10::start1_bit);

    p1.select = true;
    source.apply_input(0, p1);
    CHECK(source.machine().input_system ==
          static_cast<std::uint8_t>(M10::start1_bit | M10::coin1_bit));

    const auto state = source.save_state();
    REQUIRE_FALSE(state.empty());

    irem::irem_m10_adapter restored(make_m10_program(), "Tiny M10");
...
TEST_CASE("irem_m10_adapter drains first-pass beeper audio", "[irem_m10]") {
    irem::irem_m10_adapter adapter(make_m10_program(), "Audio M10");
    adapter.step_one_frame();

    const auto audio = adapter.drain_audio();
    CHECK(audio.frame_count > 0U);
    CHECK(audio.sample_rate == M10::audio_rate_hz);
    CHECK(audio.samples != nullptr);
}

TEST_CASE("irem_m10_adapter validates real M10 ROM sets", "[irem_m10][data]") {
    const auto dir_env = environment_value("MNEMOS_M10_SET_DIR");
    if (!dir_env.has_value() || dir_env->empty()) {
        SKIP("set MNEMOS_M10_SET_DIR to directories containing the M10 zip/folder corpus");
    }
    ...
    if (!missing_sets.empty()) {
        ...
        FAIL("missing M10 artifacts: " << missing.str());
    }
    ...
    for (const auto& set_name : expected_sets) {
        ...
        irem::irem_m10_adapter adapter(read_source_bytes(source_it->second), set_name, nullptr, {},

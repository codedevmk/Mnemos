...
namespace mnemos::apps::player::adapters::irem_m102 {
    ...
    namespace {
        ...
        frontend_sdk::session_capability_info make_session_capabilities() {
            ...
            session.input_ports = {
                {.port_index = 0U,
                 ...
                 .device_id = "irem_m102.medal.panel",
                 .label = "Medal Panel"},
            };
        ...
        [[nodiscard]] std::string resident_media_crc32(const loaded_set& set) {
            ...
            std::uint32_t crc = mnemos::security::cryptography::crc32("irem_m102.resident_media.v1");
        ...
        parse_irem_decl(std::string_view text, std::string source,
            ...
            if (!parsed.ok()) {
                for (const auto& error : parsed.errors) {
                    std::fprintf(stderr, "[irem_m102] %s:%u:%u: %s\n", error.source.c_str(),
            ...
            if (parsed.value->board != "irem_m102") {
                std::fprintf(stderr, "[irem_m102] %s declares board '%s', expected 'irem_m102'\n",
                             source.c_str(), parsed.value->board.c_str());
                if (issues != nullptr) {
                    issues->push_back({source, "unsupported Irem board '" + parsed.value->board +
                                                   "' for the M102 adapter"});
                }
                return std::nullopt;
            }
        ...
        embedded_decl_for_set(std::string_view set_name) {
            const std::string_view toml = M102::game_manifest_toml(set_name);
            if (toml.empty()) {
                return std::nullopt;
            }
            return parse_irem_decl(toml, "embedded:irem_m102/" + std::string(set_name) + ".toml",
                                   set_name);
        }
        ...
        load_declared_set(mnemos::manifests::common::rom_set_decl decl,
            ...
            for (const auto& issue : result.image.issues) {
                std::fprintf(stderr, "[irem_m102] %s: %s\n", issue.file.c_str(),
                             issue.message.c_str());
            }
            return result;
        }
        ...
        [[nodiscard]] loaded_set load_set(std::vector<std::uint8_t> rom,
            ...
            if (has_zip_signature(rom)) {
                if (auto provider =
                    ...
                    if (auto bytes = (*provider)("ga20.bin")) {
                        result.image.regions.emplace("ga20", std::move(*bytes));
                    }
                }
                return result;
            }
        ...
        [[nodiscard]] std::unique_ptr<M102::m102_system> assemble_from(loaded_set set) {
            return M102::assemble_m102(std::move(set.image), M102::board_params_for(set.set_name));
        }

        constexpr std::uint32_t irem_m102_adapter_state_version = 1U;
        constexpr std::uint32_t irem_m102_adapter_save_target_manifest_rev = 1U;
    ...
    irem_m102_adapter::irem_m102_adapter(std::vector<std::uint8_t> rom, std::string display_name,
        ...
        chip_view_ = {&sys_->main_cpu, &sys_->video, &sys_->ga20};
        publish_memory_views();
        const std::string game_label =
            !loaded_set_name_.empty()
                ? loaded_set_name_
                : (display_name.empty() ? std::string{"unknown"} : display_name);
        spec_ = {{"System", "Arcade"}, {"Board", "Irem M102"}, {"Game", game_label}};
    }

    void irem_m102_adapter::publish_memory_views() {
        ...
        publish(2U, "medal_ram", sys_->medal_ram);
    }

    void irem_m102_adapter::sync_inputs_from_ports() noexcept {
        const frontend_sdk::controller_state& c = ports_[0];
        std::uint8_t in0 = M102::input0_default;
        std::uint8_t in1 = M102::input1_default;
        if (c.select) {
            in0 &= static_cast<std::uint8_t>(~M102::input_coin1_bit);
        }
        if (c.service || c.test) {
            in0 &= static_cast<std::uint8_t>(~M102::input_service_bit);
        }
        if (c.start) {
            in0 &= static_cast<std::uint8_t>(~M102::input_start1_bit);
        }
        if (c.a) {
            in1 &= static_cast<std::uint8_t>(~M102::input_button1_bit);
        }
        if (c.b) {
            in1 &= static_cast<std::uint8_t>(~M102::input_button2_bit);
        }
        if (c.c) {
            in1 &= static_cast<std::uint8_t>(~M102::input_button3_bit);
        }
        sys_->set_inputs(in0, in1);
    }

    void irem_m102_adapter::save_adapter_state(chips::state_writer& writer) const {
        writer.u32(irem_m102_adapter_state_version);
        writer.u64(frames_stepped_);
        save_controller_state(writer, ports_[0]);
    }

    void irem_m102_adapter::load_adapter_state(chips::state_reader& reader) {
        if (reader.u32() != irem_m102_adapter_state_version) {
            reader.fail();
            return;
        }
    ...
    void irem_m102_adapter::step_one_frame() {
        sys_->run_frame();
        ++frames_stepped_;
    }

    void irem_m102_adapter::apply_input(int port,
    ...
    frontend_sdk::audio_chunk irem_m102_adapter::drain_audio() noexcept {
        const std::size_t pending = sys_->ga20.pending_samples();
        if (pending == 0U) {
            return {};
        }
        audio_buf_.assign(pending * 2U, 0);
        const std::size_t drained = sys_->ga20.drain_samples(audio_buf_.data(), pending);
        if (drained == 0U) {
            return {};
        }
        return {.samples = audio_buf_.data(),
                .frame_count = static_cast<std::uint32_t>(drained),
                .sample_rate = M102::audio_rate_hz};
    }

    std::vector<std::uint8_t> irem_m102_adapter::save_state() {
        return runtime::write_save_state(build_save_target(*this));
    }

    runtime::load_result irem_m102_adapter::load_state(std::span<const std::uint8_t> data) {
    ...
    runtime::save_target build_save_target(manifests::irem_m102::m102_system& sys) {
        runtime::save_target target;
        target.manifest_id = "irem_m102";
        target.manifest_rev = manifests::irem_m102::m102_system_state_version;
    ...
    runtime::save_target build_save_target(irem_m102_adapter& adapter) {
        runtime::save_target target;
        target.manifest_id = "irem_m102.adapter";
        target.manifest_rev = irem_m102_adapter_save_target_manifest_rev;
        target.components.push_back(
            {"board",
        ...
        const auto register_irem_m102 = [] {
            mnemos::frontend_sdk::adapter_registry::instance().register_family(
                ...
                [](mnemos::frontend_sdk::adapter_options opts)
                    ...
                    return std::make_unique<irem_m102_adapter>(

...
namespace mnemos::apps::player::adapters::irem_m102 {
    ...
    class irem_m102_adapter;

    [[nodiscard]] runtime::save_target build_save_target(manifests::irem_m102::m102_system& sys);
    [[nodiscard]] runtime::save_target build_save_target(irem_m102_adapter& adapter);

    class irem_m102_adapter final : public frontend_sdk::player_system {
      public:
        explicit irem_m102_adapter(std::vector<std::uint8_t> rom, std::string display_name = {},
        ...
        [[nodiscard]] frontend_sdk::video_region region() const noexcept override {
            return {.frames_per_second_x1000 = manifests::irem_m102::frame_rate_x1000,
        ...
        [[nodiscard]] manifests::irem_m102::m102_system& machine() noexcept { return *sys_; }
        [[nodiscard]] const std::string& set_name() const noexcept { return loaded_set_name_; }

      private:
        friend runtime::save_target build_save_target(irem_m102_adapter& adapter);
        ...
        std::unique_ptr<manifests::irem_m102::m102_system> sys_;
        std::vector<chips::ichip*> chip_view_{};
        std::array<std::unique_ptr<instrumentation::span_memory_view>, 3> memory_view_storage_{};
        std::array<instrumentation::memory_view*, 3> system_mem_view_{};
        std::array<frontend_sdk::controller_state, 1> ports_{};

...
namespace mnemos::apps::player::adapters::irem_m47 {
    ...
    namespace {
        ...
        frontend_sdk::session_capability_info make_session_capabilities() {
            ...
            session.input_ports = {
                {.port_index = 0U,
                 ...
                 .device_id = "irem_m47.panel.p1",
                 .label = "Player 1 Panel"},
                {.port_index = 1U,
                 .player_slot = 2U,
                 .format = frontend_sdk::input_device_format::arcade_panel,
                 .device_id = "irem_m47.panel.p2",
        ...
        [[nodiscard]] std::string resident_media_crc32(const loaded_set& set) {
            ...
            std::uint32_t crc = mnemos::security::cryptography::crc32("irem_m47.resident_media.v1");
        ...
        parse_irem_decl(std::string_view text, std::string source,
            ...
            if (!parsed.ok()) {
                for (const auto& error : parsed.errors) {
                    std::fprintf(stderr, "[irem_m47] %s:%u:%u: %s\n", error.source.c_str(),
            ...
            if (parsed.value->board != "irem_m47") {
                std::fprintf(stderr, "[irem_m47] %s declares board '%s', expected 'irem_m47'\n",
        ...
        embedded_decl_for_set(std::string_view set_name) {
            const std::string_view toml = M47::game_manifest_toml(set_name);
            if (toml.empty()) {
                return std::nullopt;
            }
            return parse_irem_decl(toml, "embedded:irem_m47/" + std::string(set_name) + ".toml",
                                   set_name);
        }
        ...
        load_declared_set(mnemos::manifests::common::rom_set_decl decl,
            ...
            for (const auto& issue : result.image.issues) {
                std::fprintf(stderr, "[irem_m47] %s: %s\n", issue.file.c_str(),
                             issue.message.c_str());
            }
            return result;
        }
        ...
        [[nodiscard]] std::unique_ptr<M47::m47_system> assemble_from(loaded_set set) {
            M47::m47_board_params params = M47::board_params_for(set.set_name);
            ...
            return M47::assemble_m47(std::move(set.image), params);
        }

        constexpr std::uint32_t irem_m47_adapter_state_version = 1U;
        constexpr std::uint32_t irem_m47_adapter_save_target_manifest_rev = 1U;
    ...
    irem_m47_adapter::irem_m47_adapter(std::vector<std::uint8_t> rom, std::string display_name,
        ...
        spec_ = {{"System", "Arcade"}, {"Board", "Irem M47"}, {"Game", game_label}};
        if (!dip_switches_.empty()) {
            spec_.push_back({"DIP switches", std::to_string(dip_switches_.size())});
        }
    }

    void irem_m47_adapter::publish_memory_views() {
    ...
    void irem_m47_adapter::sync_inputs_from_ports() noexcept {
    ...
    void irem_m47_adapter::save_adapter_state(chips::state_writer& writer) const {
        writer.u32(irem_m47_adapter_state_version);
    ...
    void irem_m47_adapter::load_adapter_state(chips::state_reader& reader) {
        if (reader.u32() != irem_m47_adapter_state_version) {
            reader.fail();
            return;
        }
    ...
    void irem_m47_adapter::step_one_frame() {
        sys_->run_frame();
        ++frames_stepped_;
    }

    void irem_m47_adapter::apply_input(int port,
    ...
    frontend_sdk::audio_chunk irem_m47_adapter::drain_audio() noexcept {
        ...
        return {.samples = audio_buf_.data(),
                ...
                .sample_rate = M47::audio_rate_hz};
    }

    std::vector<std::uint8_t> irem_m47_adapter::save_state() {
        return runtime::write_save_state(build_save_target(*this));
    }

    runtime::load_result irem_m47_adapter::load_state(std::span<const std::uint8_t> data) {
    ...
    runtime::save_target build_save_target(manifests::irem_m47::m47_system& sys) {
        runtime::save_target target;
        target.manifest_id = "irem_m47";
        target.manifest_rev = manifests::irem_m47::m47_system_state_version;
    ...
    runtime::save_target build_save_target(irem_m47_adapter& adapter) {
        runtime::save_target target;
        target.manifest_id = "irem_m47.adapter";
        target.manifest_rev = irem_m47_adapter_save_target_manifest_rev;
        target.components.push_back(
            {"board",
        ...
        const auto register_irem_m47 = [] {
            mnemos::frontend_sdk::adapter_registry::instance().register_family(
                ...
                [](mnemos::frontend_sdk::adapter_options opts)
                    ...
                    return std::make_unique<irem_m47_adapter>(

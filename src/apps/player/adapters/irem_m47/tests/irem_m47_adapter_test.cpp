...
namespace {
    ...
    namespace irem = mnemos::apps::player::adapters::irem_m47;
    ...
    struct temp_set_dir final {
        ...
        temp_set_dir() {
            ...
            path = fs::temp_directory_path() / ("mnemos_m47_adapter_" + std::to_string(ticks));
    ...
    [[nodiscard]] std::vector<std::uint8_t> synthetic_m47_program() {
        std::vector<std::uint8_t> rom(M47::main_rom_size, 0x00U);
    ...
    [[nodiscard]] std::vector<std::uint8_t> synthetic_m47_sound_program() {
        std::vector<std::uint8_t> rom(M47::sound_rom_size, 0x00U);
...
size = 0x2000
        ...
        write_binary_file(dir.path / "maincpu.bin", synthetic_m47_program());
        write_binary_file(dir.path / "audiocpu.bin", synthetic_m47_sound_program());
        return dir;
    }
...
size = 0x8000
    ...
    find_M47_sets(const std::vector<std::string>& roots) {
        ...
        for (const std::string& root : roots) {
            ...
            for (const fs::path& path : candidates) {
                ...
                if (stem == "olibochu" || stem == "oli-boo-chu_arcade_en") {
                    result.try_emplace("olibochu", path);
                } else if (stem == "punchkid" || stem == "punching-kid_arcade_ja") {
                    result.try_emplace("punchkid", path);
                }
            }
        }
        return result;
    ...
    [[nodiscard]] bool spec_has(const irem::irem_m47_adapter& adapter, std::string_view label,
...
TEST_CASE("irem_m47_adapter boots a synthetic M47 program", "[irem_m47]") {
    irem::irem_m47_adapter adapter(synthetic_m47_program(), "Tiny M47");
    CHECK(adapter.region().frames_per_second_x1000 == M47::frame_rate_x1000);
...
TEST_CASE("irem_m47_adapter applies manifest DIP defaults and override", "[irem_m47]") {
    const auto set = make_dip_metadata_set();
    irem::irem_m47_adapter adapter({}, "DIP metadata", nullptr, {}, set.path.string());

    CHECK(adapter.dip_switches().size() == 2U);
    CHECK(static_cast<unsigned>(adapter.machine().dsw1) == M47::m47_dsw1_default);
    CHECK(static_cast<unsigned>(adapter.machine().dsw2) == M47::m47_dsw2_default);
    CHECK(spec_has(adapter, "DIP switches", "2"));

    irem::irem_m47_adapter overridden({}, "DIP metadata", nullptr,
...
TEST_CASE("irem_m47_adapter maps service and operator-test inputs to the system port",
          "[irem_m47]") {
    irem::irem_m47_adapter adapter(synthetic_m47_program(), "M47 inputs");
...
TEST_CASE("irem_m47_adapter preserves adapter and board state", "[irem_m47]") {
    irem::irem_m47_adapter source(synthetic_m47_program(), "Save M47");
    ...
    irem::irem_m47_adapter restored(synthetic_m47_program(), "Save M47");
...
TEST_CASE("irem_m47_adapter drains mixed AY PSG samples", "[irem_m47]") {
    const auto set = make_audio_metadata_set();
    irem::irem_m47_adapter adapter({}, "Audio M47", nullptr, {}, set.path.string());
    adapter.step_one_frame();
    CHECK(adapter.machine().ay0.volume(0) == 0x0FU);
    CHECK(adapter.machine().ay1.volume(1) == 0x0CU);
    const auto audio = adapter.drain_audio();
    CHECK(audio.sample_rate == M47::audio_rate_hz);
...
TEST_CASE("irem_m47_adapter validates real M47 ROM sets", "[irem_m47][data]") {
    const auto dir_env = environment_value("MNEMOS_M47_SET_DIR");
    if (!dir_env.has_value()) {
        SKIP("set MNEMOS_M47_SET_DIR to directories containing the M47 zip/folder corpus");
    }
    const auto found = find_M47_sets(split_paths(*dir_env));
    const auto it = found.find("olibochu");
    if (it == found.end()) {
        SKIP("MNEMOS_M47_SET_DIR does not contain a Oli-Boo-Chu parent wrapper");
    }

    auto bytes = mnemos::io::read_file(it->second.string());
    REQUIRE(bytes.has_value());
    irem::irem_m47_adapter adapter(std::move(*bytes), "Oli-Boo-Chu", nullptr, {},
                                   it->second.string());
    REQUIRE(adapter.set_name() == "olibochu");
    ...
    CHECK(adapter.machine().dsw1 == M47::m47_dsw1_default);
    CHECK(adapter.machine().dsw2 == M47::m47_dsw2_default);
    ...
    if (const auto clone_it = found.find("punchkid"); clone_it != found.end()) {
        auto clone_bytes = mnemos::io::read_file(clone_it->second.string());
        REQUIRE(clone_bytes.has_value());
        irem::irem_m47_adapter clone(std::move(*clone_bytes), "Oli-Boo-Chu (Japan)", nullptr, {},
                                     clone_it->second.string());
        REQUIRE(clone.set_name() == "punchkid");
        ...
        CHECK(clone.machine().dsw1 == M47::m47_dsw1_default);
        CHECK(clone.machine().dsw2 == M47::m47_dsw2_default);
        clone.step_one_frame();
        CHECK(nonblack_pixels(clone.current_frame()) > 0U);
    }
}

TEST_CASE("irem_m47_adapter matches optional visual/audio parity hashes for a real M47 set",
          "[irem_m47][data]") {
    ...
    const auto set_env = environment_value("MNEMOS_M47_PARITY_SET");
    if (!set_env.has_value()) {
        SKIP("set MNEMOS_M47_PARITY_SET to a reference-captured M47 set");
    }
    const auto expected_frame_hash = environment_value("MNEMOS_M47_PARITY_FRAME_SHA256");
    const auto expected_audio_hash = environment_value("MNEMOS_M47_PARITY_AUDIO_SHA256");
    if (!expected_frame_hash.has_value() && !expected_audio_hash.has_value()) {
        SKIP("set MNEMOS_M47_PARITY_FRAME_SHA256 and/or MNEMOS_M47_PARITY_AUDIO_SHA256");
    }

    const int frames = env_positive_int("MNEMOS_M47_PARITY_FRAMES", 600);
    ...
    auto run_once = [&](bool want_audio) -> parity_result {
        ...
        REQUIRE(bytes.has_value());
        irem::irem_m47_adapter adapter(std::move(*bytes), "M47-parity", nullptr, {},
                                       *set_env);
    ...
    if (expected_audio_hash.has_value()) {
        ...
        CHECK(first.sample_rate == M47::audio_rate_hz);

...
namespace {
    ...
    namespace irem = mnemos::apps::player::adapters::irem_m85;
    ...
    [[nodiscard]] std::optional<std::string>
    identify_m85_source(const std::filesystem::path& path,
    ...
    [[nodiscard]] bool preferred_m85_source(const std::filesystem::path& candidate,
    ...
    index_m85_source_roots(const std::vector<std::filesystem::path>& roots,
                           const std::set<std::string, std::less<>>& known) {
        std::map<std::string, std::filesystem::path, std::less<>> sources;

        auto maybe_add = [&](const std::filesystem::path& path) {
            auto set_id = identify_m85_source(path, known);
            if (!set_id.has_value()) {
                return;
            }
            ...
            } else if (preferred_m85_source(path, existing->second)) {
    ...
    [[nodiscard]] bool spec_has(const irem::irem_m85_adapter& adapter, std::string_view label,
...
TEST_CASE("irem_m85_adapter boots a synthetic M85 program", "[irem_m85]") {
    irem::irem_m85_adapter adapter(synthetic_m85_program(), "Tiny M85");

    CHECK(adapter.region().frames_per_second_x1000 == m85::frame_rate_x1000);
    CHECK(adapter.current_frame().width == m85::visible_width);
    CHECK(adapter.current_frame().height == m85::visible_height);
    CHECK(adapter.media_capabilities().media.size() == 1U);
    CHECK(validation_issue_count(adapter.media_capabilities()) == 0U);
    CHECK(spec_has(adapter, "Board", "Irem M85"));
...
TEST_CASE("irem_m85_adapter save state restores board and adapter state", "[irem_m85]") {
    irem::irem_m85_adapter source(synthetic_m85_program(), "Tiny M85");
    ...
    irem::irem_m85_adapter restored(synthetic_m85_program(), "Tiny M85");
...
TEST_CASE("irem_m85_adapter validates real M85 ROM sets", "[irem_m85][data]") {
    ...
    const auto sources = index_m85_source_roots(roots, expected_sets);
    ...
    for (const auto& set_name : expected_sets) {
        ...
        irem::irem_m85_adapter adapter(read_source_bytes(source_path), set_name, nullptr, {},
                                       source_path.string());
        CHECK(adapter.set_name() == set_name);
        CHECK(adapter.machine().main_cpu.cpu_model() == mnemos::chips::cpu::v30::model::v30);

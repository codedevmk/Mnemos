...
namespace mnemos::apps::player::adapters::irem_m85 {
    ...
    namespace {
        ...
        frontend_sdk::session_capability_info make_session_capabilities() {
            ...
            session.input_ports = {
                {.port_index = 0U,
                 ...
                 .device_id = "irem_m85.panel.p1",
                 .label = "Player 1 Panel"},
                {.port_index = 1U,
                 .player_slot = 2U,
                 .format = frontend_sdk::input_device_format::arcade_panel,
                 .device_id = "irem_m85.panel.p2",
        ...
        [[nodiscard]] std::string resident_media_crc32(const loaded_set& set) {
            ...
            std::uint32_t crc = mnemos::security::cryptography::crc32("irem_m85.resident_media.v1");
        ...
        [[nodiscard]] std::uint16_t dip_default_from_manifest(
            std::span<const mnemos::manifests::common::rom_set_dip_switch> switches,
        ...
        parse_irem_decl(std::string_view text, std::string source,
                        ...
                        std::string_view expected_board = "irem_m85") {
            auto parsed = mnemos::manifests::common::parse_rom_set_decl(text, source);
            if (!parsed.ok()) {
                for (const auto& error : parsed.errors) {
                    std::fprintf(stderr, "[irem_m85] %s:%u:%u: %s\n", error.source.c_str(),
            ...
            if (parsed.value->board != expected_board) {
                std::fprintf(stderr, "[irem_m85] %s declares board '%s', expected '%.*s'\n",
                             source.c_str(), parsed.value->board.c_str(),
                             static_cast<int>(expected_board.size()), expected_board.data());
                if (issues != nullptr) {
        ...
        embedded_decl_for_set(std::string_view set_name) {
            const std::string_view toml = m85::game_manifest_toml(set_name);
            if (toml.empty()) {
                return std::nullopt;
            }
            return parse_irem_decl(toml, "embedded:irem_m85/" + std::string(set_name) + ".toml",
                                   set_name);
        }

        [[nodiscard]] std::optional<mnemos::manifests::common::rom_set_decl>
        embedded_parent_decl_for_set(std::string_view set_name) {
            return embedded_decl_for_set(set_name);
        }
        ...
        with_parent_fallback(const mnemos::manifests::common::rom_file_provider& clone,
            ...
            if (auto manifest_bytes = (*parent_provider)("game.toml")) {
                ...
                std::vector<rom_load_issue> m85_issues;
                resolved.decl = parse_irem_decl(text, parent_source + "/game.toml",
                                                std::string_view{parent}, &m85_issues);
                if (!resolved.decl.has_value()) {
                    for (auto& issue : m85_issues) {
        ...
        load_declared_set(mnemos::manifests::common::rom_set_decl decl,
            ...
            for (const auto& issue : result.image.issues) {
                std::fprintf(stderr, "[irem_m85] %s: %s\n", issue.file.c_str(),
                             issue.message.c_str());
            }
            return result;
        }
        ...
        [[nodiscard]] std::unique_ptr<m85::m85_system> assemble_from(loaded_set set) {
            auto params = m85::board_params_for(set.set_name);
            params.dip_default = dip_default_from_manifest(set.dip_switches, params.dip_default);
            return m85::assemble_m85(std::move(set.image), params);
        }
        ...
        constexpr std::uint32_t irem_m85_adapter_state_version = 2U;
        constexpr std::uint32_t irem_m85_adapter_save_target_manifest_rev = 1U;
    ...
    irem_m85_adapter::irem_m85_adapter(std::vector<std::uint8_t> rom, std::string display_name,
        ...
        spec_ = {{"System", "Arcade"}, {"Board", "Irem M85"}, {"Game", game_label}};
        if (!dip_switches_.empty()) {
            spec_.push_back({"DIP switches", std::to_string(dip_switches_.size())});
        }
    }

    void irem_m85_adapter::publish_memory_views() {
    ...
    void irem_m85_adapter::sync_inputs_from_ports() noexcept {
    ...
    void irem_m85_adapter::save_adapter_state(chips::state_writer& writer) const {
        writer.u32(irem_m85_adapter_state_version);
    ...
    void irem_m85_adapter::load_adapter_state(chips::state_reader& reader) {
        const std::uint32_t version = reader.u32();
        if (version == 0U || version > irem_m85_adapter_state_version) {
            reader.fail();
            return;
        }
    ...
    void irem_m85_adapter::step_one_frame() {
        sys_->run_frame();
        ++frames_stepped_;
    }

    void irem_m85_adapter::apply_input(int port,
    ...
    frontend_sdk::audio_chunk irem_m85_adapter::drain_audio() noexcept {
    ...
    std::vector<std::uint8_t> irem_m85_adapter::save_state() {
        return runtime::write_save_state(build_save_target(*this));
    }

    runtime::load_result irem_m85_adapter::load_state(std::span<const std::uint8_t> data) {
    ...
    runtime::save_target build_save_target(manifests::irem_m85::m85_system& sys) {
        runtime::save_target target;
        target.manifest_id = "irem_m85";
        target.manifest_rev = manifests::irem_m85::m85_system_state_version;
    ...
    runtime::save_target build_save_target(irem_m85_adapter& adapter) {
        runtime::save_target target;
        target.manifest_id = "irem_m85.adapter";
        target.manifest_rev = irem_m85_adapter_save_target_manifest_rev;
        target.components.push_back(
            {"board",
        ...
        const auto register_irem_m85 = [] {
            mnemos::frontend_sdk::adapter_registry::instance().register_family(
                ...
                [](mnemos::frontend_sdk::adapter_options opts)
                    ...
                    return std::make_unique<irem_m85_adapter>(

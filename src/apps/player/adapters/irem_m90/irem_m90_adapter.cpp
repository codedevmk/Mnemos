...
namespace mnemos::apps::player::adapters::irem_m90 {
    ...
    namespace {
        ...
        struct parent_resolution final {
            rom_file_provider provider;
            std::optional<rom_set_decl> decl;
        ...
        frontend_sdk::session_capability_info make_session_capabilities() {
            ...
            session.input_ports = {
                {.port_index = 0U,
                 ...
                 .device_id = "irem_m90.panel.p1",
                 .label = "Player 1 Panel"},
                {.port_index = 1U,
                 .player_slot = 2U,
                 .format = frontend_sdk::input_device_format::arcade_panel,
                 .device_id = "irem_m90.panel.p2",
        ...
        [[nodiscard]] std::string resident_media_crc32(const loaded_set& set) {
            ...
            std::uint32_t crc = mnemos::security::cryptography::crc32("irem_m90.resident_media.v1");
        ...
        parse_irem_decl(std::string_view text, std::string source,
            ...
            if (!parsed.ok()) {
                for (const auto& error : parsed.errors) {
                    std::fprintf(stderr, "[irem_m90] %s:%u:%u: %s\n", error.source.c_str(),
            ...
            if (parsed.value->board != "irem_m90") {
                std::fprintf(stderr, "[irem_m90] %s declares board '%s', expected 'irem_m90'\n",
        ...
        [[nodiscard]] std::optional<rom_set_decl> embedded_decl_for_set(std::string_view set_name) {
            const std::string_view toml = m90::game_manifest_toml(set_name);
            if (toml.empty()) {
                return std::nullopt;
            }
            return parse_irem_decl(toml, "embedded:irem_m90/" + std::string(set_name) + ".toml",
                                   set_name);
        }

        [[nodiscard]] parent_resolution with_parent_fallback(const rom_file_provider& clone,
            ...
            if (rom_path.empty()) {
                std::fprintf(stderr,
                             "[irem_m90] set declares parent '%s' but no path is known to "
            ...
            if (!is_plain_set_id(parent)) {
                std::fprintf(stderr,
                             "[irem_m90] refusing to resolve parent '%s': not a plain set id\n",
                             parent.c_str());
                resolved.issues.push_back(
            ...
            auto nested_parent = make_single_nested_zip_provider_from_path(parent_zip_path, parent);
            ...
            if (!parent_provider.has_value()) {
                std::fprintf(stderr, "[irem_m90] parent set %s or %s: %s\n",
        ...
        [[nodiscard]] loaded_set load_declared_set(rom_set_decl decl, rom_file_provider provider,
                                                   const std::string& rom_path) {
            loaded_set result;
            rom_file_provider effective = std::move(provider);
            std::vector<rom_load_issue> parent_issues;
            if (decl.parent.has_value()) {
                parent_resolution parent = with_parent_fallback(effective, *decl.parent, rom_path);
        ...
        [[nodiscard]] loaded_set load_set(std::vector<std::uint8_t> rom,
            ...
            if (has_zip_signature(rom)) {
                if (auto provider =
                    ...
                    for (const char* region : {"maincpu", "soundcpu", "samples", "graphics"}) {
        ...
        constexpr std::uint32_t irem_m90_adapter_state_version = 2U;
        constexpr std::uint32_t irem_m90_adapter_save_target_manifest_rev = 1U;
    ...
    irem_m90_adapter::irem_m90_adapter(std::vector<std::uint8_t> rom, std::string display_name,
    ...
    void irem_m90_adapter::publish_memory_views() {
        ...
        publish(4U, "rowscroll_ram", sys_->rowscroll_ram);
        publish(5U, "sound_ram", sys_->sound_ram);
    }

    void irem_m90_adapter::sync_inputs_from_ports() noexcept {
    ...
    void irem_m90_adapter::save_adapter_state(chips::state_writer& writer) const {
        writer.u32(irem_m90_adapter_state_version);
    ...
    void irem_m90_adapter::load_adapter_state(chips::state_reader& reader) {
        const std::uint32_t version = reader.u32();
        if (version == 0U || version > irem_m90_adapter_state_version) {
            reader.fail();
            return;
        }
    ...
    void irem_m90_adapter::step_one_frame() {
        sys_->run_frame();
        ++frames_stepped_;
    }

    void irem_m90_adapter::apply_input(int port,
    ...
    frontend_sdk::audio_chunk irem_m90_adapter::drain_audio() noexcept {
    ...
    std::vector<std::uint8_t> irem_m90_adapter::save_state() {
        return runtime::write_save_state(build_save_target(*this));
    }

    runtime::load_result irem_m90_adapter::load_state(std::span<const std::uint8_t> data) {
    ...
    runtime::save_target build_save_target(manifests::irem_m90::m90_system& sys) {
        runtime::save_target target;
        target.manifest_id = "irem_m90";
        target.manifest_rev = manifests::irem_m90::m90_system_state_version;
    ...
    runtime::save_target build_save_target(irem_m90_adapter& adapter) {
        runtime::save_target target;
        target.manifest_id = "irem_m90.adapter";
        target.manifest_rev = irem_m90_adapter_save_target_manifest_rev;
        target.components.push_back(
            {"board",
        ...
        const auto register_irem_m90 = [] {
            mnemos::frontend_sdk::adapter_registry::instance().register_family(
                ...
                [](mnemos::frontend_sdk::adapter_options opts)
                    ...
                    return std::make_unique<irem_m90_adapter>(

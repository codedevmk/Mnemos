...
namespace {
    ...
    namespace irem = mnemos::apps::player::adapters::irem_m57;
    namespace M57 = mnemos::manifests::irem_m57;

    [[nodiscard]] std::vector<std::uint8_t> make_m57_program() {
    ...
    [[nodiscard]] std::set<std::string, std::less<>> embedded_set_names() {
        ...
        for (const auto& [set_name, _] : M57::embedded::game_manifests) {
...
TEST_CASE("irem_m57_adapter boots a synthetic M57 program", "[irem_m57]") {
    irem::irem_m57_adapter adapter(make_m57_program(), "Tiny M57");
    CHECK(adapter.region().frames_per_second_x1000 == M57::frame_rate_x1000);
    CHECK(adapter.region().orientation == mnemos::frontend_sdk::display_orientation::horizontal);
    CHECK(adapter.current_frame().width == M57::visible_width);
    CHECK(adapter.current_frame().height == M57::visible_height);
...
TEST_CASE("irem_m57_adapter save state restores board and adapter state", "[irem_m57]") {
    irem::irem_m57_adapter source(make_m57_program(), "Tiny M57");
    ...
    CHECK(source.machine().input_p1 ==
          static_cast<std::uint8_t>(M57::panel_button1_bit | M57::panel_left_bit));
    CHECK(source.machine().input_system == M57::start1_bit);

    p1.select = true;
    source.apply_input(0, p1);
    CHECK(source.machine().input_system ==
          static_cast<std::uint8_t>(M57::start1_bit | M57::coin1_bit));

    const auto state = source.save_state();
    REQUIRE_FALSE(state.empty());

    irem::irem_m57_adapter restored(make_m57_program(), "Tiny M57");
...
TEST_CASE("irem_m57_adapter drains first-pass beeper audio", "[irem_m57]") {
    irem::irem_m57_adapter adapter(make_m57_program(), "Audio M57");
    adapter.step_one_frame();

    const auto audio = adapter.drain_audio();
    CHECK(audio.frame_count > 0U);
    CHECK(audio.sample_rate == M57::audio_rate_hz);
    CHECK(audio.samples != nullptr);
}

TEST_CASE("irem_m57_adapter validates real M57 ROM sets", "[irem_m57][data]") {
    const auto dir_env = environment_value("MNEMOS_M57_SET_DIR");
    if (!dir_env.has_value() || dir_env->empty()) {
        SKIP("set MNEMOS_M57_SET_DIR to directories containing the M57 zip/folder corpus");
    }
    ...
    if (!missing_sets.empty()) {
        ...
        FAIL("missing M57 artifacts: " << missing.str());
    }
    ...
    for (const auto& set_name : expected_sets) {
        ...
        irem::irem_m57_adapter adapter(read_source_bytes(source_it->second), set_name, nullptr, {},

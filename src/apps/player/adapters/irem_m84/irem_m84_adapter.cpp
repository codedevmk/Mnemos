...
namespace mnemos::apps::player::adapters::irem_m84 {
    ...
    namespace {
        ...
        frontend_sdk::session_capability_info make_session_capabilities() {
            ...
            session.input_ports = {
                {.port_index = 0U,
                 ...
                 .device_id = "irem_m84.panel.p1",
                 .label = "Player 1 Panel"},
                {.port_index = 1U,
                 .player_slot = 2U,
                 .format = frontend_sdk::input_device_format::arcade_panel,
                 .device_id = "irem_m84.panel.p2",
        ...
        [[nodiscard]] std::string resident_media_crc32(const loaded_set& set) {
            ...
            std::uint32_t crc = mnemos::security::cryptography::crc32("irem_m84.resident_media.v1");
        ...
        parse_irem_decl(std::string_view text, std::string source,
                        ...
                        std::string_view expected_board = "irem_m84") {
            auto parsed = mnemos::manifests::common::parse_rom_set_decl(text, source);
            if (!parsed.ok()) {
                for (const auto& error : parsed.errors) {
                    std::fprintf(stderr, "[irem_m84] %s:%u:%u: %s\n", error.source.c_str(),
            ...
            if (parsed.value->board != expected_board) {
                std::fprintf(stderr, "[irem_m84] %s declares board '%s', expected '%.*s'\n",
        ...
        embedded_decl_for_set(std::string_view set_name) {
            const std::string_view toml = m84::game_manifest_toml(set_name);
            if (toml.empty()) {
                return std::nullopt;
            }
            return parse_irem_decl(toml, "embedded:irem_m84/" + std::string(set_name) + ".toml",
                                   set_name);
        }

        [[nodiscard]] std::optional<mnemos::manifests::common::rom_set_decl>
        embedded_m81_parent_decl_for_set(std::string_view set_name) {
    ...
    irem_m84_adapter::irem_m84_adapter(std::vector<std::uint8_t> rom, std::string display_name,
    ...
    void irem_m84_adapter::sync_inputs_from_ports() noexcept {
    ...
    void irem_m84_adapter::save_adapter_state(chips::state_writer& writer) const {
        writer.u32(irem_m84_adapter_state_version);
    ...
    void irem_m84_adapter::step_one_frame() {
        sys_->run_frame();
        ++frames_stepped_;
    }

    void irem_m84_adapter::apply_input(int port,
    ...
    frontend_sdk::audio_chunk irem_m84_adapter::drain_audio() noexcept {
    ...
    std::vector<std::uint8_t> irem_m84_adapter::save_state() {
        return runtime::write_save_state(build_save_target(*this));
    }

    runtime::load_result irem_m84_adapter::load_state(std::span<const std::uint8_t> data) {
    ...
    runtime::save_target build_save_target(manifests::irem_m84::m84_system& sys) {
        runtime::save_target target;
        target.manifest_id = "irem_m84";
        target.manifest_rev = manifests::irem_m84::m84_system_state_version;
    ...
    runtime::save_target build_save_target(irem_m84_adapter& adapter) {
        runtime::save_target target;
        target.manifest_id = "irem_m84.adapter";
        target.manifest_rev = irem_m84_adapter_save_target_manifest_rev;
        target.components.push_back(
            {"board",
        ...
        const auto register_irem_m84 = [] {
            mnemos::frontend_sdk::adapter_registry::instance().register_family(
                ...
                [](mnemos::frontend_sdk::adapter_options opts)
                    ...
                    return std::make_unique<irem_m84_adapter>(

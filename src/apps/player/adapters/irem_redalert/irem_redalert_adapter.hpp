...
namespace mnemos::apps::player::adapters::irem_redalert {
    ...
    class irem_redalert_adapter;

    [[nodiscard]] runtime::save_target
    build_save_target(manifests::irem_redalert::redalert_system& sys);
    [[nodiscard]] runtime::save_target build_save_target(irem_redalert_adapter& adapter);

    class irem_redalert_adapter final : public frontend_sdk::player_system {
      public:
        explicit irem_redalert_adapter(std::vector<std::uint8_t> rom,
        ...
        [[nodiscard]] frontend_sdk::video_region region() const noexcept override {
            return {.frames_per_second_x1000 = manifests::irem_redalert::frame_rate_x1000,
        ...
        [[nodiscard]] manifests::irem_redalert::redalert_system& machine() noexcept {
            return *sys_;
        }
        [[nodiscard]] const std::string& set_name() const noexcept { return loaded_set_name_; }

      private:
        friend runtime::save_target build_save_target(irem_redalert_adapter& adapter);
        ...
        std::unique_ptr<manifests::irem_redalert::redalert_system> sys_;

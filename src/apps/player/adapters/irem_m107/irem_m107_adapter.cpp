...
namespace mnemos::apps::player::adapters::irem_m107 {
    ...
    namespace {
        ...
        frontend_sdk::session_capability_info make_session_capabilities() {
            ...
            session.input_ports = {
                {.port_index = 0U,
                 ...
                 .device_id = "irem_m107.panel.p1",
                 .label = "Player 1 Panel"},
                {.port_index = 1U,
                 .player_slot = 2U,
                 .format = frontend_sdk::input_device_format::arcade_panel,
                 .device_id = "irem_m107.panel.p2",
        ...
        parse_irem_decl(std::string_view text, std::string source,
            ...
            if (!parsed.ok()) {
                for (const auto& error : parsed.errors) {
                    std::fprintf(stderr, "[irem_m107] %s:%u:%u: %s\n", error.source.c_str(),
            ...
            if (parsed.value->board != "irem_m107") {
                std::fprintf(stderr, "[irem_m107] %s declares board '%s', expected 'irem_m107'\n",
                             source.c_str(), parsed.value->board.c_str());
                if (issues != nullptr) {
                    issues->push_back({source, "unsupported Irem board '" + parsed.value->board +
                                                   "' for the M107 adapter"});
                }
                return std::nullopt;
            }
        ...
        embedded_decl_for_set(std::string_view set_name) {
            const std::string_view toml = m107::game_manifest_toml(set_name);
            if (toml.empty()) {
                return std::nullopt;
            }
            return parse_irem_decl(toml, "embedded:irem_m107/" + std::string(set_name) + ".toml",
                                   set_name);
        }
        ...
        [[nodiscard]] loaded_set load_declared_set(
            ...
            for (const auto& issue : result.image.issues) {
                std::fprintf(stderr, "[irem_m107] %s: %s\n", issue.file.c_str(),
                             issue.message.c_str());
            }
            return result;
        }
    ...
    irem_m107_adapter::irem_m107_adapter(std::vector<std::uint8_t> rom, std::string display_name,
        ...
        spec_ = {{"System", "Arcade"}, {"Board", "Irem M107"}, {"Game", game_label}};
    }

    void irem_m107_adapter::publish_memory_views() {
    ...
    void irem_m107_adapter::save_adapter_state(chips::state_writer& writer) const {
        writer.u32(irem_m107_adapter_state_version);
    ...
    void irem_m107_adapter::step_one_frame() {
        sys_->run_frame();
        ++frames_stepped_;
    }

    void irem_m107_adapter::apply_input(int port,
    ...
    frontend_sdk::audio_chunk irem_m107_adapter::drain_audio() noexcept {
    ...
    std::vector<std::uint8_t> irem_m107_adapter::save_state() {
        return runtime::write_save_state(build_save_target(*this));
    }

    runtime::load_result irem_m107_adapter::load_state(std::span<const std::uint8_t> data) {
    ...
    runtime::save_target build_save_target(manifests::irem_m107::m107_system& sys) {
        runtime::save_target target;
        target.manifest_id = "irem_m107";
        target.manifest_rev = manifests::irem_m107::m107_system_state_version;
    ...
    runtime::save_target build_save_target(irem_m107_adapter& adapter) {
        runtime::save_target target;
        target.manifest_id = "irem_m107.adapter";
        target.manifest_rev = irem_m107_adapter_save_target_manifest_rev;
        target.components.push_back(
            {"board",
        ...
        const auto register_irem_m107 = [] {
            mnemos::frontend_sdk::adapter_registry::instance().register_family(
                ...
                [](mnemos::frontend_sdk::adapter_options opts)
                    ...
                    return std::make_unique<irem_m107_adapter>(

...
namespace mnemos::apps::player::adapters::irem_travrusa {
    ...
    class irem_travrusa_adapter;

    [[nodiscard]] runtime::save_target build_save_target(manifests::irem_travrusa::travrusa_system& sys);
    [[nodiscard]] runtime::save_target build_save_target(irem_travrusa_adapter& adapter);

    class irem_travrusa_adapter final : public frontend_sdk::player_system {
      public:
        explicit irem_travrusa_adapter(std::vector<std::uint8_t> rom, std::string display_name = {},
        ...
        [[nodiscard]] frontend_sdk::video_region region() const noexcept override {
            return {.frames_per_second_x1000 = manifests::irem_travrusa::frame_rate_x1000,
        ...
        [[nodiscard]] manifests::irem_travrusa::travrusa_system& machine() noexcept { return *sys_; }
      ...
      private:
        friend runtime::save_target build_save_target(irem_travrusa_adapter& adapter);
        ...
        std::unique_ptr<manifests::irem_travrusa::travrusa_system> sys_;
        ...
        std::vector<std::int16_t> msm_buf_{};
    };

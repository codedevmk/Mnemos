...
namespace mnemos::apps::player::adapters::irem_m58 {
    ...
    namespace {
        ...
        frontend_sdk::session_capability_info make_session_capabilities() {
            ...
            session.input_ports = {
                {.port_index = 0U,
                 ...
                 .device_id = "irem_m58.panel.p1",
                 .label = "Player 1 Panel"},
                {.port_index = 1U,
                 .player_slot = 2U,
                 .format = frontend_sdk::input_device_format::arcade_panel,
                 .device_id = "irem_m58.panel.p2",
        ...
        [[nodiscard]] std::string resident_media_crc32(const loaded_set& set) {
            ...
            std::uint32_t crc = mnemos::security::cryptography::crc32("irem_m58.resident_media.v1");
        ...
        parse_irem_decl(std::string_view text, std::string source,
            ...
            if (!parsed.ok()) {
                for (const auto& error : parsed.errors) {
                    std::fprintf(stderr, "[irem_m58] %s:%u:%u: %s\n", error.source.c_str(),
            ...
            if (parsed.value->board != "irem_m58") {
                std::fprintf(stderr, "[irem_m58] %s declares board '%s', expected 'irem_m58'\n",
        ...
        embedded_decl_for_set(std::string_view set_name) {
            const std::string_view toml = m58::game_manifest_toml(set_name);
            if (toml.empty()) {
                return std::nullopt;
            }
            return parse_irem_decl(toml, "embedded:irem_m58/" + std::string(set_name) + ".toml",
                                   set_name);
        }
        ...
        with_parent_fallback(const mnemos::manifests::common::rom_file_provider& clone,
            ...
            if (!parent_source.has_value()) {
                parent_source = find_supplemental_parent(supplemental_sources, parent);
            }
            if (!parent_source.has_value()) {
                resolved.issues.push_back(
        ...
        load_declared_set(mnemos::manifests::common::rom_set_decl decl,
            ...
            for (const auto& issue : result.image.issues) {
                std::fprintf(stderr, "[irem_m58] %s: %s\n", issue.file.c_str(),
                             issue.message.c_str());
            }
            return result;
        }
        ...
        [[nodiscard]] std::unique_ptr<m58::m58_system> assemble_from(loaded_set set) {
            m58::m58_board_params params = m58::board_params_for(set.set_name);
            ...
            return m58::assemble_m58(std::move(set.image), params);
        }

        constexpr std::uint32_t irem_m58_adapter_state_version = 1U;
        constexpr std::uint32_t irem_m58_adapter_save_target_manifest_rev = 1U;
    ...
    irem_m58_adapter::irem_m58_adapter(std::vector<std::uint8_t> rom, std::string display_name,
    ...
    void irem_m58_adapter::publish_memory_views() {
        ...
        publish(1U, "color_ram", sys_->color_ram);
        publish(2U, "sprite_ram", sys_->sprite_ram);
        publish(3U, "work_ram", sys_->work_ram);
    }

    void irem_m58_adapter::sync_inputs_from_ports() noexcept {
    ...
    void irem_m58_adapter::save_adapter_state(chips::state_writer& writer) const {
        writer.u32(irem_m58_adapter_state_version);
    ...
    void irem_m58_adapter::load_adapter_state(chips::state_reader& reader) {
        if (reader.u32() != irem_m58_adapter_state_version) {
            reader.fail();
            return;
        }
    ...
    void irem_m58_adapter::step_one_frame() {
        sys_->run_frame();
        ++frames_stepped_;
    }

    void irem_m58_adapter::apply_input(int port,
    ...
    frontend_sdk::audio_chunk irem_m58_adapter::drain_audio() noexcept {
        const std::size_t pending = std::max(sys_->ay0.pending_samples(),
        ...
        return {.samples = audio_buf_.data(),
                ...
                .sample_rate = m58::audio_rate_hz};
    }

    std::vector<std::uint8_t> irem_m58_adapter::save_state() {
        return runtime::write_save_state(build_save_target(*this));
    }

    runtime::load_result irem_m58_adapter::load_state(std::span<const std::uint8_t> data) {
    ...
    runtime::save_target build_save_target(manifests::irem_m58::m58_system& sys) {
        runtime::save_target target;
        target.manifest_id = "irem_m58";
        target.manifest_rev = manifests::irem_m58::m58_system_state_version;
    ...
    runtime::save_target build_save_target(irem_m58_adapter& adapter) {
        runtime::save_target target;
        target.manifest_id = "irem_m58.adapter";
        target.manifest_rev = irem_m58_adapter_save_target_manifest_rev;
        target.components.push_back(
            {"board",
        ...
        const auto register_irem_m58 = [] {
            mnemos::frontend_sdk::adapter_registry::instance().register_family(
                ...
                [](mnemos::frontend_sdk::adapter_options opts)
                    ...
                    return std::make_unique<irem_m58_adapter>(

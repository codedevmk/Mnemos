...
namespace {
    ...
    namespace irem = mnemos::apps::player::adapters::irem_m58;
    ...
    struct temp_set_dir final {
        ...
        temp_set_dir() {
            ...
            path = fs::temp_directory_path() / ("mnemos_m58_adapter_" + std::to_string(ticks));
    ...
    [[nodiscard]] std::vector<std::uint8_t> synthetic_m58_program() {
        std::vector<std::uint8_t> rom(m58::main_rom_size, 0x00U);
    ...
    [[nodiscard]] std::vector<std::uint8_t> synthetic_m58_sound_program() {
        std::vector<std::uint8_t> rom(m58::sound_rom_size, 0x01U);
        const std::uint8_t program[] = {
            0x96U, 0x02U, // LDAA <$02
            0x97U, 0x06U, // STAA <$06
            0x86U, 0x07U, 0x97U, 0x00U, 0x86U, 0xFEU, 0x97U, 0x01U, 0x86U, 0x08U, 0x97U,
            0x00U, 0x86U, 0x0FU, 0x97U, 0x01U, 0x86U, 0x00U, 0x97U, 0x00U, 0x86U, 0x1FU,
            0x97U, 0x01U, 0x86U, 0x07U, 0x97U, 0x04U, 0x86U, 0xFDU, 0x97U, 0x05U, 0x86U,
            0x09U, 0x97U, 0x04U, 0x86U, 0x0CU, 0x97U, 0x05U, 0x86U, 0x02U, 0x97U, 0x04U,
            0x86U, 0x23U, 0x97U, 0x05U, 0x7EU, 0x80U, 0x00U};
...
size = 0x10000
        ...
        write_binary_file(dir.path / "maincpu.bin", synthetic_m58_program());
        write_binary_file(dir.path / "soundcpu.bin", synthetic_m58_sound_program());
        return dir;
    }
...
size = 0x10000
    ...
    find_m58_sets(const std::vector<std::string>& roots) {
        ...
        for (const std::string& root : roots) {
            ...
            for (const fs::path& path : candidates) {
                ...
                if (stem == "10yard" || stem == "10-yard-fight_arcade_en") {
                    result.try_emplace("10yard", path);
                } else if (stem == "10yardj" || stem == "10-yard-fight_arcade_ja") {
                    result.try_emplace("10yardj", path);
                } else if (stem == "vs10yard" || stem == "vs-10-yard-fight_arcade_en") {
                    result.try_emplace("vs10yard", path);
                } else if (stem == "vs10yardj" || stem == "vs-10-yard-fight_arcade_ja") {
                    result.try_emplace("vs10yardj", path);
                }
            }
        }
        return result;
    ...
    [[nodiscard]] bool spec_has(const irem::irem_m58_adapter& adapter, std::string_view label,
...
TEST_CASE("irem_m58_adapter boots a synthetic M58 program", "[irem_m58]") {
    irem::irem_m58_adapter adapter(synthetic_m58_program(), "Tiny M58");
    CHECK(adapter.region().frames_per_second_x1000 == m58::frame_rate_x1000);
    ...
    CHECK(irem::build_save_target(adapter).manifest_rev == 1U);
    REQUIRE(adapter.chips().size() == 5U);
    CHECK(adapter.chips()[2]->metadata().part_number == "MC6803");
    CHECK(adapter.chips()[3]->metadata().part_number == "YM2149");
    CHECK(adapter.chips()[4]->metadata().part_number == "YM2149");
    CHECK(nonblack_pixels(adapter.current_frame()) > 0U);
}
TEST_CASE("irem_m58_adapter applies manifest DIP defaults and override", "[irem_m58]") {
    const auto set = make_dip_metadata_set();
    irem::irem_m58_adapter adapter({}, "DIP metadata", nullptr, {}, set.path.string());

    CHECK(adapter.dip_switches().size() == 2U);
    CHECK(static_cast<unsigned>(adapter.machine().dsw1) == m58::tenyard_dsw1_default);
    CHECK(static_cast<unsigned>(adapter.machine().dsw2) == m58::tenyard_dsw2_default);
    CHECK(spec_has(adapter, "DIP switches", "2"));

    irem::irem_m58_adapter overridden({}, "DIP metadata", nullptr,
...
TEST_CASE("irem_m58_adapter maps service and operator-test inputs to the system port",
          "[irem_m58]") {
    irem::irem_m58_adapter adapter(synthetic_m58_program(), "M58 inputs");
...
TEST_CASE("irem_m58_adapter preserves adapter and board state", "[irem_m58]") {
    irem::irem_m58_adapter source(synthetic_m58_program(), "Save M58");
    ...
    irem::irem_m58_adapter restored(synthetic_m58_program(), "Save M58");
...
TEST_CASE("irem_m58_adapter drains mixed AY PSG samples", "[irem_m58]") {
    const auto set = make_audio_metadata_set();
    irem::irem_m58_adapter adapter({}, "Audio M58", nullptr, {}, set.path.string());
    adapter.step_one_frame();
    CHECK(adapter.machine().ay0.volume(0) == 0x0FU);
    CHECK(adapter.machine().ay1.volume(1) == 0x0CU);
    const auto audio = adapter.drain_audio();
    CHECK(audio.sample_rate == m58::audio_rate_hz);
...
TEST_CASE("irem_m58_adapter validates real M58 ROM sets", "[irem_m58][data]") {
    const auto dir_env = environment_value("MNEMOS_M58_SET_DIR");
    if (!dir_env.has_value()) {
        SKIP("set MNEMOS_M58_SET_DIR to directories containing the M58 zip/folder corpus");
    }
    const auto found = find_m58_sets(split_paths(*dir_env));
    const auto it = found.find("10yard");
    if (it == found.end()) {
        SKIP("MNEMOS_M58_SET_DIR does not contain a 10-Yard Fight parent wrapper");
    }

    auto bytes = mnemos::io::read_file(it->second.string());
    REQUIRE(bytes.has_value());
    irem::irem_m58_adapter adapter(std::move(*bytes), "10-Yard Fight", nullptr, {},
                                   it->second.string());
    REQUIRE(adapter.set_name() == "10yard");
    ...
    CHECK(adapter.machine().dsw1 == m58::tenyard_dsw1_default);
    CHECK(adapter.machine().dsw2 == m58::tenyard_dsw2_default);
    ...
    if (const auto clone_it = found.find("10yardj"); clone_it != found.end()) {
        auto clone_bytes = mnemos::io::read_file(clone_it->second.string());
        REQUIRE(clone_bytes.has_value());
        irem::irem_m58_adapter clone(std::move(*clone_bytes), "10-Yard Fight (Japan)", nullptr, {},
                                     clone_it->second.string());
        REQUIRE(clone.set_name() == "10yardj");
        REQUIRE_FALSE(clone.media_capabilities().media.empty());
        CHECK(clone.media_capabilities().media.front().validation_issues.empty());
        CHECK(clone.dip_switches().empty());
        CHECK(clone.machine().dsw1 == m58::tenyard_dsw1_default);
        CHECK(clone.machine().dsw2 == m58::tenyard_dsw2_default);
        clone.step_one_frame();
        CHECK(nonblack_pixels(clone.current_frame()) > 0U);
    }
}

TEST_CASE("irem_m58_adapter matches optional visual/audio parity hashes for a real M58 set",
          "[irem_m58][data]") {
    ...
    const auto set_env = environment_value("MNEMOS_M58_PARITY_SET");
    if (!set_env.has_value()) {
        SKIP("set MNEMOS_M58_PARITY_SET to a reference-captured M58 set");
    }
    const auto expected_frame_hash = environment_value("MNEMOS_M58_PARITY_FRAME_SHA256");
    const auto expected_audio_hash = environment_value("MNEMOS_M58_PARITY_AUDIO_SHA256");
    if (!expected_frame_hash.has_value() && !expected_audio_hash.has_value()) {
        SKIP("set MNEMOS_M58_PARITY_FRAME_SHA256 and/or MNEMOS_M58_PARITY_AUDIO_SHA256");
    }

    const int frames = env_positive_int("MNEMOS_M58_PARITY_FRAMES", 600);
    ...
    auto run_once = [&](bool want_audio) -> parity_result {
        ...
        REQUIRE(bytes.has_value());
        irem::irem_m58_adapter adapter(std::move(*bytes), "M58-parity", nullptr, {},
                                       *set_env);
    ...
    if (expected_audio_hash.has_value()) {
        ...
        CHECK(first.sample_rate == m58::audio_rate_hz);

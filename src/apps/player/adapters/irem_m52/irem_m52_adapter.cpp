...
namespace mnemos::apps::player::adapters::irem_m52 {
    ...
    namespace {
        ...
        frontend_sdk::session_capability_info make_session_capabilities() {
            ...
            session.input_ports = {
                {.port_index = 0U,
                 ...
                 .device_id = "irem_m52.panel.p1",
                 .label = "Player 1 Panel"},
                {.port_index = 1U,
                 .player_slot = 2U,
                 .format = frontend_sdk::input_device_format::arcade_panel,
                 .device_id = "irem_m52.panel.p2",
        ...
        [[nodiscard]] std::string resident_media_crc32(const loaded_set& set) {
            ...
            std::uint32_t crc = mnemos::security::cryptography::crc32("irem_m52.resident_media.v1");
        ...
        parse_irem_decl(std::string_view text, std::string source,
            ...
            if (!parsed.ok()) {
                for (const auto& error : parsed.errors) {
                    std::fprintf(stderr, "[irem_m52] %s:%u:%u: %s\n", error.source.c_str(),
            ...
            if (parsed.value->board != "irem_m52") {
                std::fprintf(stderr, "[irem_m52] %s declares board '%s', expected 'irem_m52'\n",
        ...
        embedded_decl_for_set(std::string_view set_name) {
            const std::string_view toml = m52::game_manifest_toml(set_name);
            if (toml.empty()) {
                return std::nullopt;
            }
            return parse_irem_decl(toml, "embedded:irem_m52/" + std::string(set_name) + ".toml",
                                   set_name);
        }
        ...
        constexpr std::uint32_t irem_m52_adapter_save_target_manifest_rev = 1U;
    ...
    irem_m52_adapter::irem_m52_adapter(std::vector<std::uint8_t> rom, std::string display_name,
        ...
        chip_view_ = {&sys_->video, &sys_->main_cpu, &sys_->audio_probe};
    ...
    void irem_m52_adapter::sync_inputs_from_ports() noexcept {
    ...
    void irem_m52_adapter::save_adapter_state(chips::state_writer& writer) const {
        writer.u32(irem_m52_adapter_state_version);
    ...
    void irem_m52_adapter::load_adapter_state(chips::state_reader& reader) {
        if (reader.u32() != irem_m52_adapter_state_version) {
            reader.fail();
            return;
        }
    ...
    void irem_m52_adapter::step_one_frame() {
        sys_->run_frame();
        ++frames_stepped_;
    }

    void irem_m52_adapter::apply_input(int port,
    ...
    frontend_sdk::audio_chunk irem_m52_adapter::drain_audio() noexcept {
        const std::size_t pending = sys_->audio_probe.pending_samples();
        if (pending == 0U) {
            return {};
        }
        mono_buf_.assign(pending, 0);
        const std::size_t drained =
            sys_->audio_probe.drain_samples(mono_buf_.data(), mono_buf_.size());
        if (drained == 0U) {
            return {};
        }
    ...
    std::vector<std::uint8_t> irem_m52_adapter::save_state() {
        return runtime::write_save_state(build_save_target(*this));
    }

    runtime::load_result irem_m52_adapter::load_state(std::span<const std::uint8_t> data) {
    ...
    runtime::save_target build_save_target(manifests::irem_m52::m52_system& sys) {
        runtime::save_target target;
        target.manifest_id = "irem_m52";
        target.manifest_rev = manifests::irem_m52::m52_system_state_version;
    ...
    runtime::save_target build_save_target(irem_m52_adapter& adapter) {
        runtime::save_target target;
        target.manifest_id = "irem_m52.adapter";
        target.manifest_rev = irem_m52_adapter_save_target_manifest_rev;
        target.components.push_back(
            {"board",
        ...
        const auto register_irem_m52 = [] {
            mnemos::frontend_sdk::adapter_registry::instance().register_family(
                ...
                [](mnemos::frontend_sdk::adapter_options opts)
                    ...
                    return std::make_unique<irem_m52_adapter>(

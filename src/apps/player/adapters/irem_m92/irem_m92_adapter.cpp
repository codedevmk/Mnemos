...
namespace mnemos::apps::player::adapters::irem_m92 {
    ...
    namespace {
        ...
        frontend_sdk::session_capability_info make_session_capabilities() {
            ...
            session.input_ports = {
                {.port_index = 0U,
                 ...
                 .device_id = "irem_m92.panel.p1",
                 .label = "Player 1 Panel"},
                {.port_index = 1U,
                 .player_slot = 2U,
                 .format = frontend_sdk::input_device_format::arcade_panel,
                 .device_id = "irem_m92.panel.p2",
        ...
        [[nodiscard]] std::string resident_media_crc32(const loaded_set& set) {
            ...
            std::uint32_t crc = mnemos::security::cryptography::crc32("irem_m92.resident_media.v1");
        ...
        parse_irem_decl(std::string_view text, std::string source,
            ...
            if (!parsed.ok()) {
                for (const auto& error : parsed.errors) {
                    std::fprintf(stderr, "[irem_m92] %s:%u:%u: %s\n", error.source.c_str(),
            ...
            if (parsed.value->board != "irem_m92") {
                std::fprintf(stderr, "[irem_m92] %s declares board '%s', expected 'irem_m92'\n",
        ...
        embedded_decl_for_set(std::string_view set_name) {
            const std::string_view toml = m92::game_manifest_toml(set_name);
            if (toml.empty()) {
                return std::nullopt;
            }
            return parse_irem_decl(toml, "embedded:irem_m92/" + std::string(set_name) + ".toml",
                                   set_name);
        }
        ...
        with_parent_fallback(const mnemos::manifests::common::rom_file_provider& clone,
            ...
            if (rom_path.empty()) {
                std::fprintf(stderr,
                             "[irem_m92] set declares parent '%s' but no path is known to "
            ...
            if (!is_plain_set_id(parent)) {
                std::fprintf(stderr,
                             "[irem_m92] refusing to resolve parent '%s': not a plain set id\n",
                             parent.c_str());
                resolved.issues.push_back(
            ...
            if (!parent_provider.has_value()) {
                std::fprintf(stderr, "[irem_m92] parent set %s or %s: %s\n",
        ...
        load_declared_set(mnemos::manifests::common::rom_set_decl decl,
            ...
            for (const auto& issue : result.image.issues) {
                std::fprintf(stderr, "[irem_m92] %s: %s\n", issue.file.c_str(),
                             issue.message.c_str());
            }
            return result;
        }
            ...
            if (has_zip_signature(rom)) {
                if (auto provider =
                    ...
                    for (const char* region :
                         {"maincpu", "soundcpu", "tiles", "sprites", "samples", "plds"}) {
        ...
        [[nodiscard]] std::unique_ptr<m92::m92_system> assemble_from(loaded_set set) {
            return m92::assemble_m92(std::move(set.image), m92::board_params_for(set.set_name));
        }
        ...
        constexpr std::uint32_t irem_m92_adapter_state_version = 2U;
        constexpr std::uint32_t irem_m92_adapter_save_target_manifest_rev = 1U;
    ...
    irem_m92_adapter::irem_m92_adapter(std::vector<std::uint8_t> rom, std::string display_name,
        ...
        chip_view_ = {&sys_->video, &sys_->main_cpu, &sys_->sound_cpu, &sys_->fm, &sys_->pcm};
        publish_memory_views();
        const std::string game_label =
            !loaded_set_name_.empty()
                ? loaded_set_name_
                : (display_name.empty() ? std::string{"unknown"} : display_name);
        spec_ = {{"System", "Arcade"}, {"Board", "Irem M92"}, {"Game", game_label}};
    }

    void irem_m92_adapter::publish_memory_views() {
    ...
    void irem_m92_adapter::sync_inputs_from_ports() noexcept {
    ...
    void irem_m92_adapter::save_adapter_state(chips::state_writer& writer) const {
        writer.u32(irem_m92_adapter_state_version);
    ...
    void irem_m92_adapter::load_adapter_state(chips::state_reader& reader) {
        const std::uint32_t version = reader.u32();
        if (version == 0U || version > irem_m92_adapter_state_version) {
            reader.fail();
            return;
        }
    ...
    void irem_m92_adapter::step_one_frame() {
        sys_->run_frame();
        ++frames_stepped_;
    }

    void irem_m92_adapter::apply_input(int port,
    ...
    frontend_sdk::audio_chunk irem_m92_adapter::drain_audio() noexcept {
    ...
    std::vector<std::uint8_t> irem_m92_adapter::save_state() {
        return runtime::write_save_state(build_save_target(*this));
    }

    runtime::load_result irem_m92_adapter::load_state(std::span<const std::uint8_t> data) {
    ...
    runtime::save_target build_save_target(manifests::irem_m92::m92_system& sys) {
        runtime::save_target target;
        target.manifest_id = "irem_m92";
        target.manifest_rev = manifests::irem_m92::m92_system_state_version;
    ...
    runtime::save_target build_save_target(irem_m92_adapter& adapter) {
        runtime::save_target target;
        target.manifest_id = "irem_m92.adapter";
        target.manifest_rev = irem_m92_adapter_save_target_manifest_rev;
        target.components.push_back(
            {"board",

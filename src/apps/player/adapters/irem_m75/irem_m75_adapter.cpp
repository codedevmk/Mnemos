...
namespace mnemos::apps::player::adapters::irem_m75 {
    ...
    namespace {
        ...
        frontend_sdk::session_capability_info make_session_capabilities() {
            ...
            session.input_ports = {
                {.port_index = 0U,
                 ...
                 .device_id = "irem_m75.panel.p1",
                 .label = "Player 1 Panel"},
                {.port_index = 1U,
                 .player_slot = 2U,
                 .format = frontend_sdk::input_device_format::arcade_panel,
                 .device_id = "irem_m75.panel.p2",
        ...
        [[nodiscard]] std::string resident_media_crc32(const loaded_set& set) {
            ...
            std::uint32_t crc = mnemos::security::cryptography::crc32("irem_m75.resident_media.v1");
        ...
        parse_irem_decl(std::string_view text, std::string source,
            ...
            if (!parsed.ok()) {
                for (const auto& error : parsed.errors) {
                    std::fprintf(stderr, "[irem_m75] %s:%u:%u: %s\n", error.source.c_str(),
            ...
            if (parsed.value->board != "irem_m75") {
                std::fprintf(stderr, "[irem_m75] %s declares board '%s', expected 'irem_m75'\n",
        ...
        embedded_decl_for_set(std::string_view set_name) {
            const std::string_view toml = m75::game_manifest_toml(set_name);
            if (toml.empty()) {
                return std::nullopt;
            }
            return parse_irem_decl(toml, "embedded:irem_m75/" + std::string(set_name) + ".toml",
                                   set_name);
        }
        ...
        [[nodiscard]] std::unique_ptr<m75::m75_system> assemble_from(loaded_set set) {
            m75::m75_board_params params = m75::board_params_for(set.set_name);
            ...
            return m75::assemble_m75(std::move(set.image), params);
        }
        ...
        constexpr std::uint32_t irem_m75_adapter_state_version = 2U;
        constexpr std::uint32_t irem_m75_adapter_save_target_manifest_rev = 1U;
    ...
    irem_m75_adapter::irem_m75_adapter(std::vector<std::uint8_t> rom, std::string display_name,
        ...
        spec_ = {{"System", "Arcade"}, {"Board", "Irem M75"}, {"Game", game_label}};
        if (!dip_switches_.empty()) {
            spec_.push_back({"DIP switches", std::to_string(dip_switches_.size())});
        }
    }

    void irem_m75_adapter::publish_memory_views() {
        ...
        publish(3U, "video_ram", sys_->video_ram);
        publish(4U, "sound_ram", sys_->sound_ram);
    }

    void irem_m75_adapter::sync_inputs_from_ports() noexcept {
    ...
    void irem_m75_adapter::save_adapter_state(chips::state_writer& writer) const {
        writer.u32(irem_m75_adapter_state_version);
    ...
    void irem_m75_adapter::load_adapter_state(chips::state_reader& reader) {
        const std::uint32_t version = reader.u32();
        if (version == 0U || version > irem_m75_adapter_state_version) {
            reader.fail();
            return;
        }
    ...
    void irem_m75_adapter::step_one_frame() {
        sys_->run_frame();
        ++frames_stepped_;
    }

    void irem_m75_adapter::apply_input(int port,
    ...
    frontend_sdk::audio_chunk irem_m75_adapter::drain_audio() noexcept {
    ...
    std::vector<std::uint8_t> irem_m75_adapter::save_state() {
        return runtime::write_save_state(build_save_target(*this));
    }

    runtime::load_result irem_m75_adapter::load_state(std::span<const std::uint8_t> data) {
    ...
    runtime::save_target build_save_target(manifests::irem_m75::m75_system& sys) {
        runtime::save_target target;
        target.manifest_id = "irem_m75";
        target.manifest_rev = manifests::irem_m75::m75_system_state_version;
    ...
    runtime::save_target build_save_target(irem_m75_adapter& adapter) {
        runtime::save_target target;
        target.manifest_id = "irem_m75.adapter";
        target.manifest_rev = irem_m75_adapter_save_target_manifest_rev;
        target.components.push_back(
            {"board",
        ...
        const auto register_irem_m75 = [] {
            mnemos::frontend_sdk::adapter_registry::instance().register_family(
                ...
                [](mnemos::frontend_sdk::adapter_options opts)
                    ...
                    return std::make_unique<irem_m75_adapter>(

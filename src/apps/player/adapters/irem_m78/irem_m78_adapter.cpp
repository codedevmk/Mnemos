...
namespace mnemos::apps::player::adapters::irem_m78 {
    ...
    namespace {
        ...
        [[nodiscard]] std::string resident_media_crc32(const loaded_set& set) {
            ...
            std::uint32_t crc = mnemos::security::cryptography::crc32("irem_m78.resident_media.v1");
        ...
        [[nodiscard]] std::optional<rom_set_decl>
        parse_irem_decl(std::string_view text, std::string source,
            ...
            if (!parsed.ok()) {
                for (const auto& error : parsed.errors) {
                    std::fprintf(stderr, "[irem_m78] %s:%u:%u: %s\n", error.source.c_str(),
            ...
            if (parsed.value->board != "irem_m78") {
                std::fprintf(stderr, "[irem_m78] %s declares board '%s', expected 'irem_m78'\n",
        ...
        [[nodiscard]] std::optional<rom_set_decl> embedded_decl_for_set(std::string_view set_name) {
            const std::string_view toml = m78::game_manifest_toml(set_name);
            if (toml.empty()) {
                return std::nullopt;
            }
            return parse_irem_decl(toml, "embedded:irem_m78/" + std::string(set_name) + ".toml",
                                   set_name);
        }

        [[nodiscard]] loaded_set load_declared_set(rom_set_decl decl, rom_file_provider provider) {
            loaded_set result;
            ...
            for (const auto& issue : result.image.issues) {
                std::fprintf(stderr, "[irem_m78] %s: %s\n", issue.file.c_str(),
                             issue.message.c_str());
            }
            return result;
        }
            ...
            if (has_zip_signature(rom)) {
                if (auto provider =
                    ...
                    for (const char* region :
                         {"maincpu", "audiocpu", "tiles", "tiles2", "m72_audio", "proms"}) {
        ...
        constexpr std::uint32_t irem_m78_adapter_state_version = 1U;
        constexpr std::uint32_t irem_m78_adapter_save_target_manifest_rev = 1U;
    ...
    irem_m78_adapter::irem_m78_adapter(std::vector<std::uint8_t> rom, std::string display_name,
        ...
        sys_ = m78::assemble_m78(std::move(set.image));
        if (dip_override.has_value()) {
            sys_->dsw1 = static_cast<std::uint8_t>(*dip_override);
        }
        chip_view_ = {&sys_->video, &sys_->main_cpu, &sys_->sound_cpu, &sys_->fm, &sys_->dac};
        publish_memory_views();
        const std::string game_label =
            !loaded_set_name_.empty()
                ? loaded_set_name_
                : (display_name.empty() ? std::string{"unknown"} : display_name);
        spec_ = {{"System", "Arcade"}, {"Board", "Irem M78"}, {"Game", game_label}};
    }

    void irem_m78_adapter::publish_memory_views() {
        ...
        publish(2U, "layer0_tile_ram", sys_->tile_ram[0]);
        publish(3U, "layer0_attr_ram", sys_->attr_ram[0]);
        publish(4U, "layer0_color_ram", sys_->color_ram[0]);
        publish(5U, "layer1_tile_ram", sys_->tile_ram[1]);
        publish(6U, "vregs", sys_->vregs);
        publish(7U, "layer_control", sys_->layer_control);
    }

    void irem_m78_adapter::sync_inputs_from_ports() noexcept {
        const frontend_sdk::controller_state& c = ports_[0];
        std::uint8_t in0 = m78::m78_in0_default;
        std::uint8_t in1 = m78::m78_in1_default;
        if (c.service || c.test) {
            in0 &= static_cast<std::uint8_t>(~0x01U);
        }
        if (c.select) {
            in0 &= static_cast<std::uint8_t>(~0x02U);
        }
        if (c.start || c.a) {
            in1 |= 0x01U;
        }
    ...
    void irem_m78_adapter::save_adapter_state(chips::state_writer& writer) const {
        writer.u32(irem_m78_adapter_state_version);
        writer.u64(frames_stepped_);
        writer.u64(samples_drained_);
        save_controller_state(writer, ports_[0]);
    }

    void irem_m78_adapter::load_adapter_state(chips::state_reader& reader) {
        if (reader.u32() != irem_m78_adapter_state_version) {
            reader.fail();
            return;
        }
    ...
    void irem_m78_adapter::step_one_frame() {
        sys_->run_frame();
        ++frames_stepped_;
    }

    void irem_m78_adapter::apply_input(int port,
    ...
    frontend_sdk::audio_chunk irem_m78_adapter::drain_audio() noexcept {
        ...
        std::int16_t dac = sys_->dac.output();
    ...
    std::vector<std::uint8_t> irem_m78_adapter::save_state() {
        return runtime::write_save_state(build_save_target(*this));
    }

    runtime::load_result irem_m78_adapter::load_state(std::span<const std::uint8_t> data) {
    ...
    runtime::save_target build_save_target(manifests::irem_m78::m78_system& sys) {
        runtime::save_target target;
        target.manifest_id = "irem_m78";
        target.manifest_rev = manifests::irem_m78::m78_system_state_version;
    ...
    runtime::save_target build_save_target(irem_m78_adapter& adapter) {
        runtime::save_target target;
        target.manifest_id = "irem_m78.adapter";
        target.manifest_rev = irem_m78_adapter_save_target_manifest_rev;
        target.components.push_back(
            {"board",
        ...
        const auto register_irem_m78 = [] {
            mnemos::frontend_sdk::adapter_registry::instance().register_family(
                ...
                [](mnemos::frontend_sdk::adapter_options opts)
                    ...
                    return std::make_unique<irem_m78_adapter>(
